% generated from papers/XVII-function-fields/paper_XVII_function_fields.tex -- edit the paper, then rerun assemble.py
\chapter[Bost--Connes systems over function fields]{Localized Bost--Connes systems over global function fields:\\ commensurability forbids type $\mathrm{III}_1$,\\ and the factor recovers the temperature}\label{ch:XVII}
\chapterorigin{This chapter is Paper XVII of the series (\texttt{papers/XVII-function-fields/}).}
\begin{summary}
...
inverts and, with it, the central negative result of Chapter~\ref{ch:IX}.

Over a number field the logarithms $\log\Ns\pp$ of an infinite prime set are never pairwise
commensurable [Ch.~\ref{ch:I}, Lem.~4.1]; over a function field they are \emph{always}
...
[Ch.~\ref{ch:I}, Thm.~3.2], where $\mathrm{III}_1$ is generic. The proposed alternative
...
non-isomorphic; so \textbf{the von Neumann algebra determines $\beta\,d_S\log q$}. Chapter~\ref{ch:IX}
...
Chapter~\ref{ch:IX} in the whole programme, and it is bought by commensurability rather
...
\]
Constructions and notation follow [Ch.One convention is needed at once: for $S=\cP_K$ the degree
map gives $\mathbb I_S/\overline{K^\times_S}$ a quotient $\Z$, so the group as written is
not compact; as the framework of [Ch.~\ref{ch:Main}] requires a compact symmetry group (and as is
...
\begin{proposition}[Periodicity]\label{XVII:prop:period}
...
\begin{remark}\label{XVII:rem:periodic}
...
\begin{proposition}[Weil zeta]\label{XVII:prop:zeta}
With $T=q^{-\beta}$,
\[
...
\begin{corollary}[Critical exponent]\label{XVII:cor:betac}
...
\begin{remark}[Numerical illustration]\label{XVII:rem:numbetac}
...
\begin{lemma}[Total commensurability]\label{XVII:lem:commens}
...
\begin{remark}[The inversion]\label{XVII:rem:inversion}
[Ch.~\ref{ch:I}, Lem.~4.1] proves that over a number field an infinite $S$ never has pairwise
...
\begin{theorem}[Never $\mathrm{III}_1$]\label{XVII:thm:neverIII1}
...
\begin{proof}
By [Ch.~\ref{ch:Main}, Prop.~6.1], $S(M)\subseteq\overline{\Gamma_\beta}\cup\{0\}$, and
$\overline{\Gamma_\beta}=\Gamma_\beta$ by Lemma~\ref{XVII:lem:commens}. Type $\mathrm{III}_1$
...
\begin{corollary}[Contrast]\label{XVII:cor:contrast}
By [Ch.~\ref{ch:I}, Thm.~3.2], over a number field with $S=P_K$ and $0<\beta\le1$ the type is
always $\mathrm{III}_1$. Over a function field it never is, for any $S$ and any $\beta$. The
two settings are opposite in this respect, and the reason is Lemma~\ref{XVII:lem:commens} alone,
no arithmetic being used.
\end{corollary}

\section{The factor recovers the temperature}

\begin{theorem}[Partial repair of Chapter~\ref{ch:IX}]\label{XVII:thm:repair}
...
amenable --- the argument of Chapter~\ref{ch:IX} (its Lemma 1.1) applies verbatim, the acting group
...
\begin{remark}[What this changes]\label{XVII:rem:changes}
Theorem 2.1 of Chapter~\ref{ch:IX} showed that over number fields
...
temperature. Theorem~\ref{XVII:thm:repair} is the first place in this programme where that
...
striking in Chapter~\ref{ch:IX}, and it is recovered here for free, by commensurability rather
...
\begin{remark}[The remaining ambiguity]\label{XVII:rem:ambiguity}
...
separately requires the $C^*$-level, where by Proposition 1.1 of Chapter~\ref{ch:XV} the point spectrum
$\Sp_p(\sigma)=q^{d_S\Z}$ is available and pins $q^{d_S}$ directly.
\end{remark}

\section{The Weil--Chebotarev obstruction}

\begin{proposition}[Corrected bound]\label{XVII:prop:weilbound}
...
\begin{remark}[The proposed bound]\label{XVII:rem:proposedbound}
...
$O_\chi(q^{n/2}/n)$, as in Proposition~\ref{XVII:prop:weilbound}.
\end{remark}

\begin{remark}[Constant field extensions need separate treatment]\label{XVII:rem:constfield}
...
class-group characters of [Ch.~\ref{ch:Main}, \S4.1], and they must be handled directly. That is
...
(Corollary~\ref{XVII:cor:betac}).
\end{remark}

\begin{theorem}[Unconditional uniqueness]\label{XVII:thm:uniqueness}
...
obstruction of [Ch.~\ref{ch:Main}, \S4.1] has no analogue at $S=\cP_K$: the characters pulled back
...
constant-field tower, Proposition~\ref{XVII:prop:weilbound} bounds the second term by
$2(B_\chi+4g+4)q^{n/2}/n$, so the sum is $\ge a_n$ for $n$ large and
$\sum_nq^{-\beta n}a_n=\infty$ for $\beta\le1$ by Corollary~\ref{XVII:cor:betac}. If
$\chi=\zeta^{\deg}$ is a constant-field character, Remark~\ref{XVII:rem:constfield} applies.
...
[Ch.~\ref{ch:Main}, Thm.~3.9], whose Kakutani--martingale proof uses only the product structure of
...
\begin{remark}[What is gained over the number field case]\label{XVII:rem:gain}
The corresponding statement over $\Q$ [Ch.~\ref{ch:Main}, Thm.~3.9] relies on the prime ideal theorem
...
\begin{theorem}[Closed low-temperature phase, no sieve]\label{XVII:thm:sparse}
...
$[1,\infty)$, exactly as in [Ch.~\ref{ch:Main}, Thm.~5.5], and the construction uses no sieve
...
phase is [Ch.~\ref{ch:Main}, Thm.~5.5], whose hypothesis is exactly convergence at the critical
exponent.
\end{proof}

\begin{remark}[Numerical check]\label{XVII:rem:numsparse}
...
\begin{remark}[Comparison]\label{XVII:rem:compare}
Over $\Q$ the closed phase of [Ch.~\ref{ch:Main}, Thm.~5.5] was obtained for the Sophie Germain
...
\log\Ns\pp\ \text{never commensurable} & \text{always commensurable} & \text{[Ch.~\ref{ch:I}, Lem.~4.1], Lem.~3.1}\\
...
\text{unramified obstruction ([Ch.~\ref{ch:Main}, \S4.1])} & \text{none at }S=\cP_K\text{: positive density} & \text{Thm.~5.4}
\end{array}
\]

The transfer succeeds and, unlike the transfer to arithmetic topology of Chapter~\ref{ch:VIII},
...
retain the temperature that Chapter~\ref{ch:IX} showed is destroyed over number fields.

We should be exact about the size of that last claim. The factor recovers one real number,
$\beta d_S\log q$, and by Remark~\ref{XVII:rem:ambiguity} not even $q$ and $\beta$ separately. It
does not recover the curve, its genus, or the obstruction group; the collapse of
Chapter~\ref{ch:IX} is weakened, not reversed. But it is the first weakening, and it comes from
the arithmetic of the ground field rather than from any additional structure imposed on the
system.

Three questions. Which of the three cases of Theorem~\ref{XVII:thm:neverIII1} actually occurs, and
for which $S$? The natural guess is $\mathrm{III}_{q^{-\beta d_S}}$ throughout, but
determining $\Sigma_\beta$ exactly, as Chapter~\ref{ch:I} does over number fields
([Ch.~\ref{ch:I}, Prop.~2.2] for the bound via the extended group, [Ch.~\ref{ch:I}, Thm.~4.2]
...
circle action rather than a flow --- bring the classification of Chapter~\ref{ch:XIII} within reach of
...
[Ch.~\ref{ch:I}, Thm.~5.1], is open and looks tractable.
