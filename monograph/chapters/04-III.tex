% generated from papers/III-invariants/paper_III_invariants.tex -- edit the paper, then rerun assemble.py
\chapter[Invariants, entropy, and the measure class]{Invariants, entropy, and the measure class}\label{ch:III}
\renewcommand{\hO}{\widehat{\mathcal{O}}}
\chapterorigin{This chapter is Paper III of the series (\texttt{papers/III-invariants/}).}
\begin{summary}
The obstruction group $\Xi_\beta(K,S)$ of [Ch.~\ref{ch:Main}] is a complete invariant of the phase
...
resulting staircase coincides with the subfactor index staircase of Chapter~\ref{ch:II}.
...
\begin{equation}\label{III:eq:Xi}
...
product measure of [Ch.~\ref{ch:Main}, Lem.~2.10], $\alpha_\pp$ for the endomorphism
$f\mapsto\mu_\pp f\mu_\pp^*$ of $C(Y_S)$, and $\cB_{K,S}=\cA_{K,S}\rtimes_\sigma\R$ for the
continuous core.

The organizing statement is Theorem~\ref{III:thm:meas}: $\Xi_\beta$ is the kernel of a map into
...
expect them to forget $\Xi_\beta$, and \S\S\ref{III:sec:Ktheory}--\ref{III:sec:entropy} confirm this
...
\section{$\Xi_\beta$ is measurable first cohomology}\label{III:sec:meas}

Let $\sigma:\Z^{(S)}\to G_S$ be the canonical homomorphism, regarded as a cocycle on
$\cR_S$ via $\sigma(v+a,v)=\sigma_\aaa$.

\begin{lemma}[Coboundary dictionary]\label{III:lem:dict}
...
\begin{proof}
$(1)\Leftrightarrow(2)$ is Propositions 3.3 and 3.4 of [Ch.~\ref{ch:Main}]; those proofs use only
...
$(2)\Leftrightarrow(3)$ is [Ch.~\ref{ch:Main}, Lem.~3.2] together with
...
\begin{remark}\label{III:rem:noconverse}
...
For $A=G_S$ compact this is automatic, and Theorem~\ref{III:thm:meas} needs only
$(1)\Leftrightarrow(3)$. For the cocycle with values in $G_S\times\R$ that governs the
factor type, regularity is not known, and [Ch.~\ref{ch:I}, Prop.~2.2 and Rem.~2.3] obtains
from the inclusion $E(\Phi)\subseteq\Lambda^\perp$ an upper bound on the type only.
\end{remark}

\begin{theorem}[Cohomological description]\label{III:thm:meas}
For every $\beta>0$,
\begin{equation}\label{III:eq:coh}
...
\begin{proof}
Apply Lemma~\ref{III:lem:dict} with $A=G_S$, $\Phi=\sigma$: condition (3) is \eqref{III:eq:Xi}, and
(1) says the class of $\chi\circ\sigma$ vanishes in $H^1_{\mathrm{meas}}$.
\end{proof}

\begin{corollary}[Where the $\beta$-dependence sits]\label{III:cor:where}
In \eqref{III:eq:coh} the objects $\widehat{G_S}$, $\cR_S$ and $\sigma$ are $\beta$-independent.
The entire dependence on $\beta$ is carried by the measure class of $\pi_\beta$, through
which $H^1_{\mathrm{meas}}$ is computed.
\end{corollary}

\begin{remark}\label{III:rem:category}
Corollary~\ref{III:cor:where} is the reason for everything below. $K$-theory is an invariant of
...
\S\S\ref{III:sec:Ktheory}--\ref{III:sec:entropy} are therefore not failures of ingenuity but
category errors, and we record them in that spirit.
\end{remark}

\section{$K$-theory: the eigenvalue equation}\label{III:sec:Ktheory}

We first correct a statement announced in an earlier draft.

\begin{proposition}[KMS states do not act on $K_0(\cA_{K,S})$]\label{III:prop:notrace}
...
\begin{theorem}[The KMS simplex as an eigenvalue problem]\label{III:thm:eigen}
...
\begin{proof}
By [Ch.~\ref{ch:Main}, Prop.~2.8] a $\mathrm{KMS}_\beta$ state is $\nu\circ E$ for a unique
...
\begin{corollary}[The crossed product annihilates the datum]\label{III:cor:annih}
...
$\omega\circ\alpha_{\pp*}=\omega$ for every $\pp$. By Theorem~\ref{III:thm:eigen} the
...
homology, computed in Chapter~\ref{ch:II}.)
\end{corollary}

\begin{remark}\label{III:rem:pair}
Theorem~\ref{III:thm:eigen} is a statement about the \emph{pair} $\bigl(K_0(C(Y_S)),\alpha_*\bigr)$
...
\section{Homotopy invariants are blind}\label{III:sec:homotopy}

\begin{theorem}[The continuous core]\label{III:thm:core}
...
\begin{proof}
(1) is the Connes--Thom isomorphism $K_i(\cA\rtimes_\sigma\R)\cong K_{i+1}(\cA)$ \cite{ConnesThom}.
...
\begin{remark}[Two mechanisms, one cause]\label{III:rem:twomech}
...
of the commuting family $(1-\alpha_{\pp*})$, and we have not determined it (Chapter~\ref{ch:II}). Were
...
the same mechanism as in Remark~\ref{III:rem:connected}.
\end{remark}

\begin{remark}[Connected versus totally disconnected]\label{III:rem:connected}
...
\begin{proposition}[The determinant does not act on $K_1$]\label{III:prop:dlhs}
...
\begin{remark}[Parity]\label{III:rem:parity}
...
\emph{twisted} cyclic $1$-cocycle, consistently with Theorem~\ref{III:thm:meas}.
\end{remark}

\begin{proposition}[Groupoid homology carries no $\beta$]\label{III:prop:homology}
...
quasi-invariance together with [Ch.~\ref{ch:Main}, Prop.~2.8].
\end{proof}

\section{Dynamical entropy}\label{III:sec:entropy}

\begin{proposition}[Scaling, not invariance]\label{III:prop:scaling}
...
\begin{theorem}[No invariant measure, hence no metric entropy]\label{III:thm:noinv}
...
$\nu$ on $\partial Y_S=\{[0,g]\}\cong G_S/r(U_S)\cong\Cl^+_S$ [Ch.~\ref{ch:Main}, (2.1)], a finite
set. A
...
whose entropy is computed; Proposition~\ref{III:prop:scaling} shows no KMS state is.
\end{proof}

\begin{remark}[Consistency with type $\mathrm{III}$]\label{III:rem:typeIII}
Theorem~\ref{III:thm:noinv} restates the type classification. A type $\mathrm{III}$ factor has no
...
\begin{theorem}[Topological entropy vanishes]\label{III:thm:ht}
...
of Chapter~\ref{ch:II}) the map $\alpha_\aaa$ becomes the inner automorphism $\mathrm{Ad}\,\mu_\aaa$,
...
\begin{remark}[Why $\log\Ns\aaa$ does not occur]\label{III:rem:nolog}
...
$\log n$ (Chapter~\ref{ch:II}). The
unavailability of $\log\Ns\aaa$ is the same phenomenon as the failure of the Cuntz relation
there.
\end{remark}

\begin{theorem}[The modular flow has zero entropy]\label{III:thm:cnt}
...
\begin{remark}\label{III:rem:rotation}
...
$\Xi_\beta$, which by Corollary~\ref{III:cor:where} is not an invariant of an automorphism.
\end{remark}

\section{Relative entropy and the information staircase}\label{III:sec:relent}

Fix $\beta$ with $\Xi_\beta$ finite and write $G=\widehat{\Xi_\beta}$, $A_\beta$ for the set
of thresholds already crossed in the cascade systems of [Ch.~\ref{ch:Main}, Prop.~4.16], so that
$\Xi_\beta\cong(\Z/2\Z)^{A_\beta}$ and $|G|=2^{|A_\beta|}$.

\begin{theorem}[The staircase]\label{III:thm:staircase}
Let $\varphi_\varepsilon$, $\varepsilon\in G$, be the extremal $\mathrm{KMS}_\beta$ states
and $\phisym$ the symmetric one. Then
\begin{equation}\label{III:eq:staircase}
...
When $A_\beta$ is infinite the two states are mutually singular and \eqref{III:eq:staircase} is
$+\infty$.
\end{theorem}

\begin{proof}
By [Ch.~\ref{ch:Main}, Thm.~3.6] the extreme points of the $\mathrm{KMS}_\beta$ simplex are indexed
...
\begin{corollary}[One bit per threshold]\label{III:cor:bit}
Each threshold contributes exactly $\log2$ to \eqref{III:eq:staircase}, and the function
...
which is $|G|$ (Chapter~\ref{ch:II}): the index staircase and the information
staircase are the same object, both counting $\log$ of the number of phases.
\end{corollary}

\begin{remark}[The order parameter]\label{III:rem:psi}
For the single-threshold system of [Ch.~\ref{ch:Main}, Prop.~4.12] the two phases are separated by
...
threshold freezes one previously maximally mixed bit. This is \eqref{III:eq:staircase} in its
elementary form.
\end{remark}

\begin{proposition}[No preferred phase]\label{III:prop:landauer}
At fixed $\beta$ the $\mathrm{KMS}_\beta$ states form, by [Ch.~\ref{ch:Main}, Thm.~3.6], the
...
extreme points simply transitively [Ch.~\ref{ch:Main}, Thm.~3.6(3)]; the symmetries commute with
$\sigma_t$.
\end{proof}

\begin{remark}[What the bit is]\label{III:rem:landauer}
...
of \eqref{III:eq:staircase} is the information needed to say which phase one is in, not
information lost. By Proposition~\ref{III:prop:landauer} no phase is energetically preferred,
so the analogy with Landauer does not survive. (In the Gibbs regime $\zeta_{K,S}(\beta)<\infty$
one can say more: the extremal states are the normalized Gibbs states of [Ch.~\ref{ch:Main}, Thm.~3.6],
...
K_*(\cA_{K,S}) & \text{no} & \text{Cor.~\ref{III:cor:annih}: coinvariants kill the eigenvectors}\\
K_*(\cB_{K,S}) & \text{no} & \text{Thm.~\ref{III:thm:core}(1)}\\
\text{dual trace on }K_0(\cB_{K,S}) & \text{no} & \text{Thm.~\ref{III:thm:core}(3): scaling vs.\ homotopy}\\
\hat\sigma_*\ \text{on}\ K_*(\cB_{K,S}) & \text{no} & \text{Thm.~\ref{III:thm:core}(2): }\R\ \text{connected}\\
\Delta_{\hat\varphi}\ \text{on}\ K_1 & \text{not defined} & \text{Prop.~\ref{III:prop:dlhs}: domain is }\cU_0\\
H_*(\cG_S;\Z)\ \text{and }c\text{-twists} & \text{no} & \text{Prop.~\ref{III:prop:homology}: no }\beta\\
h_\varphi(\alpha_\aaa),\ h_\varphi(\cL_\aaa) & \text{not defined} & \text{Thm.~\ref{III:thm:noinv}: no invariant measure}\\
\htop(\alpha_\aaa) & 0 & \text{Thm.~\ref{III:thm:ht}: a contraction}\\
h_\varphi(\sigma_t) & 0 & \text{Thm.~\ref{III:thm:cnt}: compact gauge subflow}\\
\bigl(K_0(C(Y_S)),\alpha_*\bigr) & \textbf{yes} & \text{Thm.~\ref{III:thm:eigen}}\\
H^1_{\mathrm{meas}}(\cR_S,\pi_\beta;\T) & \textbf{yes} & \text{Thm.~\ref{III:thm:meas}}\\
\Ent(\omega_{\varphi_\varepsilon}\|\omega_{\phisym}) & \textbf{yes} & \text{Thm.~\ref{III:thm:staircase}}
\end{array}
\]

The three affirmative entries have a common form: each is built directly from a
Radon--Nikodym derivative. The eigenvalue equation of Theorem~\ref{III:thm:eigen} is the
statement that $d(\nu\circ\alpha_\pp)/d\nu=\Ns\pp^{-\beta}$; the cohomology of
Theorem~\ref{III:thm:meas} is computed with respect to the measure class of $\pi_\beta$, that is,
modulo the Radon--Nikodym cocycle; and the relative entropy of
Theorem~\ref{III:thm:staircase} is by definition
...
class, and a homotopy functor cannot see a scaling. That is the content of Corollary~\ref{III:cor:where}, and it is the reason the
...
Lemma~\ref{III:lem:dict} is stated for $\T$-valued cocycles; the extended group of
[Ch.~\ref{ch:I}], defined by cocycles valued in $G_S\times\R$, is the corresponding kernel for
the factor type, and there the inclusion $E(\Phi)\subseteq\Lambda^\perp$ of
Lemma~\ref{III:lem:dict} yields only an upper bound on the type; whether the kernel determines
the type --- i.e.\ whether the cocycle is regular, or equivalently whether the flow of weights
has pure point spectrum --- is open (Remark~\ref{III:rem:noconverse}). Third, the twisted cyclic $1$-cocycle
of Remark~\ref{III:rem:parity} is the natural home for a pairing with $K_1(\cB_{K,S})$; whether
the resulting invariant recovers Theorem~\ref{III:thm:eigen} we do not know.
