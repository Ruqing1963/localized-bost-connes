% generated from papers/XXX-affine-ktheory/paper_XXX_affine_ktheory.tex -- edit the paper, then rerun assemble.py
\chapter[K-theory of the affine boundary quotient]{K-theory of the affine boundary quotient:\\ the return of the norm and the trace}\label{ch:XXX}
\renewcommand{\Ns}{N}
\chapterorigin{This chapter is Paper XXX of the series (\texttt{papers/XXX-affine-ktheory/}).}
\begin{summary}
For the affine control system of Chapter~\ref{ch:II} (its \S6) at a \emph{principal} prime
...
survives, and the divisibility bound $(\Ns\pp-1)[1]=0$ of Chapter~\ref{ch:II} is attained; and, in the
...
$\pp$ the semigroup of Chapter~\ref{ch:II}'s \S6 is not well defined over $\OK$, and the ray-class
repair is the first of three problems recorded honestly at the end.
\end{summary}


\section{The setting, and a repair}\label{XXX:sec:setting}

Chapter~\ref{ch:II} (its \S6) attaches to a set $S$ of primes of $K$ the affine system
...
a\mathfrak a')$ leaves $\OK$: the affine monoid of Chapter~\ref{ch:II}'s \S6 is well defined over
...
\S\ref{XXX:sec:honest}; as Remark~\ref{XXX:rem:units} shows, this is exactly where the class group
enters the affine theory.

\section{The dilated form}\label{XXX:sec:reduction}

\begin{proposition}\label{XXX:prop:dilation}
...
\section{The exterior tower and the iterated Pimsner--Voiculescu computation}\label{XXX:sec:pv}

Write $B=C_0(K_\pp)\rtimes\OK[1/\pi]$ and $\alpha$ for the scaling automorphism, so that
the algebra of Proposition~\ref{XXX:prop:dilation} is $B\rtimes_\alpha\Z$.

\begin{lemma}[The tower]\label{XXX:lem:tower}
...
calibrations of Remark~\ref{XXX:rem:calibration} pin the normalization.
\end{proof}

\begin{remark}[Two calibrations, $n=1$]\label{XXX:rem:calibration}
...
\begin{theorem}[K-theory of the affine boundary, principal single prime]\label{XXX:thm:main}
...
divisibility bound of Chapter~\ref{ch:II}. The order of $T_r$ is the part of
...
summed over the parity decomposition of Lemma~\ref{XXX:lem:tower}. At $r=0$:
...
the remaining extension data being Problem (b) of \S\ref{XXX:sec:honest}.
\end{proof}

\begin{example}[Real quadratic, explicitly]\label{XXX:ex:quadratic}
...
\begin{remark}[The units are audible; the class group is at the door]\label{XXX:rem:units}
...
chosen uniformizer --- the reason Chapter~\ref{ch:II}'s \S6 had to ``fix a family of uniformizers''.
...
\section{What the affine boundary remembers, and what remains}\label{XXX:sec:honest}
