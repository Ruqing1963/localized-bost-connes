% generated from papers/XIII-equivariant/paper_XIII_equivariant.tex -- edit the paper, then rerun assemble.py
\chapter[The fixed-point algebra is the Cartan subalgebra]{Equivariant rigidity of localized Bost--Connes\\ $C^*$-dynamical systems: the fixed-point algebra\\ is the Cartan subalgebra}\label{ch:XIII}
\chapterorigin{This chapter is Paper XIII of the series (\texttt{papers/XIII-equivariant/}).}
\begin{summary}
By Chapter~\ref{ch:IX} the von Neumann algebra of a localized Bost--Connes system
...
We use the notation of [Ch.~\ref{ch:Main}]: $\cA_{K,S}=C(Y_S)\rtimes J_S$, spanned by $\mu_\aaa f\mu_\bb^*$,
...
\begin{definition}\label{XIII:def:injective}
Say that $S$ is \emph{norm-separated} if $\Ns:J_S\to\N$, $\aaa\mapsto\Ns\aaa$, is injective.
\end{definition}

\begin{lemma}[When norm-separation holds]\label{XIII:lem:sep}
...
$f_{\pp'}\log\Ns\pp-f_\pp\log\Ns\pp'=0$. Compare [Ch.~\ref{ch:I}, Lem.~4.1].
\end{proof}

\begin{example}\label{XIII:ex:fail}
...
\begin{theorem}[The Cartan subalgebra is the fixed-point algebra]\label{XIII:thm:fixed}
If $S$ is norm-separated then
\begin{equation}\label{XIII:eq:fixed}
...
\begin{remark}[Why this needs saying]\label{XIII:rem:whyneeded}
...
respect the pair. Theorem~\ref{XIII:thm:fixed} supplies exactly that, by identifying the Cartan
subalgebra intrinsically, in terms of the flow that the isomorphism is assumed to intertwine.
Without it the reconstruction argument of \S\ref{XIII:sec:rec} has a gap.
\end{remark}

\begin{remark}[The unseparated case]\label{XIII:rem:unsep}
...
The fixed-point algebra is then non-commutative and Theorem~\ref{XIII:thm:fixed} fails. This does not
...
\section{Reconstruction}\label{XIII:sec:rec}
...
\begin{lemma}[The semigroup is intrinsic]\label{XIII:lem:bisections}
...
\begin{proof}
By Lemma~\ref{XIII:lem:sep}, norm-separation means that $\Ns:I_S\to\Q_{>0}$ is injective on the
...
\begin{definition}\label{XIII:def:frobdense}
...
\begin{proposition}[The symmetry group is intrinsic over $\Q$]\label{XIII:prop:aut}
...
\begin{remark}\label{XIII:rem:autgeneral}
...
\begin{theorem}[Equivariant reconstruction]\label{XIII:thm:rec}
...
$S_i$ are Dirichlet-dense, by Proposition~\ref{XIII:prop:aut}. Then
...
\begin{proof}
(1) Theorem~\ref{XIII:thm:fixed} makes $\Phi$ an isomorphism of Cartan pairs. The groupoid
...
(3) is Lemma~\ref{XIII:lem:bisections}. (4) $\theta$ maps $B_\aaa$ to $B_{\pi(\aaa)}$, hence the
...
simplices are $\Prob(G_S/\Xi_\beta^\perp)$ by [Ch.~\ref{ch:Main}, Thm.~3.6], and an affine isomorphism
...
$\iota$-equivariant, so in the coordinates $y=(v,h)$ of [Ch.~\ref{ch:Main}, Lem.~2.10] on the points
...
[Ch.~\ref{ch:Main}, (1.1)], whose convergence class is unchanged when $\sigma_\pp$ is altered by
...
\begin{remark}[The hypotheses]\label{XIII:rem:indep}
Norm-separation enters twice, in Theorem~\ref{XIII:thm:fixed} and in
Lemma~\ref{XIII:lem:bisections}; by Lemma~\ref{XIII:lem:sep} it is the same as the $\Q$-independence
...
classes is an invariant of the dynamical system in general (Remark~\ref{XIII:rem:autgeneral}).
\end{remark}

\section{What is not recovered}

\begin{remark}[The field]\label{XIII:rem:field}
Theorem~\ref{XIII:thm:rec} recovers the pair $\bigl(S,\Ns|_S\bigr)$, and, when the symmetry
...
\begin{remark}[Arithmetic equivalence is not obviously the right equivalence]\label{XIII:rem:arith}
...
Theorem~\ref{XIII:thm:rec}(6). So the invariant recovered here is finer than the zeta function and
...
\begin{definition}\label{XIII:def:bundle}
...
\begin{proposition}[What the bundle sees]\label{XIII:prop:bundle}
...
Definition~\ref{XIII:def:bundle} determines in addition the chain $\{\Xi_\beta\}_{\beta>0}$ as
subgroups of $\widehat{G_S}$, since $\Xi_\beta^\perp$ is the stabilizer in $G_S$ of any extreme
point of the fibre at $\beta$.
\end{proposition}

\begin{proof}
Each of these is read off [Ch.~\ref{ch:Main}, Thm.~3.6]: the fibre at $\beta$ is
$\Prob(G_S/\Xi_\beta^\perp)$, on whose extreme points $G_S$ acts transitively with
stabilizer $\Xi_\beta^\perp$.
\end{proof}

\begin{remark}[What the bundle does not see]\label{XIII:rem:bundleweak}
...
groupoid of Theorem~\ref{XIII:thm:rec}, which recovers the norms outright, and the chain of
implications runs
\[
...
\begin{remark}[Flows on Kirchberg algebras]\label{XIII:rem:kirchberg}
By Chapter~\ref{ch:II} the boundary $\partial\cA^{\mathrm{aff}}_{K,S}$ of the affine
...
\S\ref{XIII:sec:rec}, by contrast, the relevant rigidity is that of continuous orbit
equivalence \cite{Li,Matsumoto}, whose groupoid form is Renault's theorem.)

Second, and more concretely, by Chapter~\ref{ch:II} this system has
...
\S\ref{XIII:sec:rec} can be carried out.
\end{remark}
...
\cA^\sigma=C(Y_S)\ \text{for norm-separated }S & \textbf{true} & \text{Thm.~\ref{XIII:thm:fixed}}\\
\text{norm-separation automatic over }\Q & \text{true} & \text{Lem.~\ref{XIII:lem:sep}}\\
\text{equivariant iso preserves the Cartan pair} & \textbf{true} & \text{Thm.~\ref{XIII:thm:fixed}}\\
\text{Renault: pair}\Rightarrow\text{groupoid} & \text{true} & \text{Thm.~\ref{XIII:thm:rec}(1)}\\
\text{norm-separation}=\Q\text{-indep.\ of }\log\Ns\pp & \text{true} & \text{Lem.~\ref{XIII:lem:sep}}\\
\text{norms }\{\Ns\pp\}\ \text{recovered} & \text{true} & \text{Thm.~\ref{XIII:thm:rec}(3)}\\
\text{simplices, transition locus, }\beta_c\ \text{recovered} & \text{true} & \text{Thm.~\ref{XIII:thm:rec}(5)}\\
G_S,\ \sigma_\pp,\ \{\Xi_\beta\}\ \text{recovered} & \text{if symmetries intertwined} & \text{Thm.~\ref{XIII:thm:rec}(6)}\\
\operatorname{Aut}_0=G_S & \text{true over }\Q\text{, Dirichlet-dense }S & \text{Prop.~\ref{XIII:prop:aut}}\\
h^+_S\ \text{recovered} & \textbf{not claimed} & \text{Rem.~\ref{XIII:rem:arith}}\\
\text{the field }K\ \text{recovered} & \textbf{not claimed} & \text{Rem.~\ref{XIII:rem:field}}\\
\text{KMS bundle a complete invariant} & \textbf{not claimed; weaker} & \text{Rem.~\ref{XIII:rem:bundleweak}}\\
\text{Kirchberg boundary classified by }KK_\R & \textbf{not available} & \text{Rem.~\ref{XIII:rem:kirchberg}}
\end{array}
\]

The contrast with Chapter~\ref{ch:IX} is the point. There the von Neumann algebra was shown to
...
Theorem~\ref{XIII:thm:fixed}: the flow itself pins down the Cartan subalgebra, so the
...
that Theorem~\ref{XIII:thm:rec} extends beyond Lemma~\ref{XIII:lem:sep}? Is
