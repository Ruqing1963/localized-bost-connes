% generated from papers/XXV-sato-tate/paper_XXV_sato_tate.tex -- edit the paper, then rerun assemble.py
\chapter[Sato--Tate families]{Sato--Tate families in localized Bost--Connes systems:\\ open phase, no congruence obstruction,\\ and the symmetric power expansion}\label{ch:XXV}
\renewcommand{\Tr}{\operatorname{tr}}
\chapterorigin{This chapter is Paper XXV of the series (\texttt{papers/XXV-sato-tate/}).}
\begin{summary}
...
is $k=1$. By Proposition 1.1 of Chapter~\ref{ch:XXIV} the critical series is then
...
opposite verdict to Theorem 2.2(1) of Chapter~\ref{ch:XXIV}, where the Landau and
...
The framework is that of [Ch.\begin{proposition}[Density]\label{XXV:prop:density}
For $0\le a<b\le\pi$,
\[
...
\begin{theorem}[Open phase]\label{XXV:thm:open}
$\beta_c(S_{[a,b]})=1$, and
\begin{equation}\label{XXV:eq:asym}
...
\begin{proof}
In the notation of Proposition 1.1 of Chapter~\ref{ch:XXIV}, $\theta=1$ and $k=1$, so $\beta_c=\theta=1$
and the critical series is $\asymp\delta\int_2^xdt/(t\log t)=\delta\log\log x\to\infty$. For
\eqref{XXV:eq:asym}, write $E(x)=\pi_S(x)-\delta\pi(x)=o(x/\log x)$ and integrate by parts:
...
convergence at $\beta_c$, hence $k>1$ by Proposition 1.1 of Chapter~\ref{ch:XXIV}.
\end{proof}

\begin{corollary}[No positive-density set has a closed phase]\label{XXV:cor:noclosed}
...
$\asymp x/\log^kx$ with $k\ge2$ --- which by Remark 1.3 of Chapter~\ref{ch:XXIV} means two
simultaneous prime conditions. Sato--Tate families, being of positive density, are on the
opposite side of that divide from the Sophie Germain family of [Ch.~\ref{ch:Main}, Thm.~5.5].
\end{corollary}

\begin{example}\label{XXV:ex:densities}
...
\begin{lemma}[The residue is the determinant]\label{XXV:lem:det}
...
\begin{theorem}[Joint equidistribution]\label{XXV:thm:joint}
...
\begin{proof}
By Lemma~\ref{XXV:lem:det} and Serre's open image theorem \cite{Serre} the mod-$m$ image is open, so the
...
\begin{theorem}[High-temperature uniqueness]\label{XXV:thm:unique}
...
\emph{no} congruence obstruction in the sense of Theorem 2.2(1) of Chapter~\ref{ch:XXIV}.
\end{theorem}

\begin{proof}
By [Ch.~\ref{ch:Main}, Thm.~3.9] uniqueness at $\beta$ requires
...
Theorem~\ref{XXV:thm:joint} that set has density $\delta(1-1/[\,\cdot\,])>0$ in the primes, so
its Dirichlet series diverges for $\beta\le1$ by Theorem~\ref{XXV:thm:open}. For $\beta>1$ the
whole of $\zeta_S(\beta)$ converges, so $\Xi_\beta=\widehat{G_S}$.
\end{proof}

\begin{remark}[Numerical verification]\label{XXV:rem:numjoint}
...
sample sizes ($167$ and $464$ primes). Contrast Remark 2.3 of Chapter~\ref{ch:XXIV}, where the twin
...
\begin{remark}[Why this family passes the test]\label{XXV:rem:whypass}
The obstructed families of Chapter~\ref{ch:XXIV} --- Landau and Friedlander--Iwaniec --- are defined by
...
Lemma~\ref{XXV:lem:det} the residue is the determinant while $a_p$ is the trace of the same
...
\begin{proposition}[Chebyshev coefficients]\label{XXV:prop:cheb}
...
\begin{theorem}[The expansion]\label{XXV:thm:expansion}
For $\Re s>1$,
\begin{equation}\label{XXV:eq:expansion}
\zeta_{S_{[a,b]}}(s)=\delta\,\log\frac{1}{s-1}+\sum_{k\ge1}c_k\log L\bigl(s,\Sym^kE\bigr)+O(1),
\end{equation}
the series over $k$ summed by truncation as in Remark~\ref{XXV:rem:smoothing}, and every term
...
\begin{remark}[The smoothing is not vacuous]\label{XXV:rem:smoothing}
$\mathbf 1_{[a,b]}$ is not continuous, so the Chebyshev series converges in $L^2(\mu_\ST)$
but not uniformly, and the interchange of $\sum_k$ with $\sum_p$ in \eqref{XXV:eq:expansion} is
...
\begin{example}[Coefficients]\label{XXV:ex:coeffs}
...
intervals pass the congruence test of Chapter~\ref{ch:XXIV} by \emph{equidistribution} --- the clean
...
The obstructed families of Chapter~\ref{ch:XXIV} --- Landau and Friedlander--Iwaniec --- fail because
...
[Ch.~\ref{ch:Main}, Thm.~5.5] found for the Sophie Germain family. By Corollary~\ref{XXV:cor:noclosed} the
...
sets defined by trace conditions, which have positive density. By the revised Theorem 2.2(2) of Chapter~\ref{ch:XXIV} the twin, Sophie Germain and Chen families all
...
Theorem 5.3 of Chapter~\ref{ch:VI} were built by letting $[a,b]$ shrink with $p$; by
Theorem~\ref{XXV:thm:open} any \emph{fixed} interval gives $\beta_c=1$ with an open phase, so the
whole of the phase-structure interest in Sato--Tate families lies in shrinking intervals,
which is where Chapter~\ref{ch:VI} placed it.
