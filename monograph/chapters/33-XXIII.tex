% generated from papers/XXIII-leopoldt/paper_XXIII_leopoldt.tex -- edit the paper, then rerun assemble.py
\chapter[Leopoldt, the strata, and $\log_p(\Ns\pp)=0$]{Leopoldt and the $p$-local Bost--Connes groupoid:\\ the rank of the isotropy, not its triviality,\\ and why $\log_p(\Ns\pp)$ vanishes}\label{ch:XXIII}
\chapterorigin{This chapter is Paper XXIII of the series (\texttt{papers/XXIII-leopoldt/}).}
\begin{summary}
Chapter~\ref{ch:XXII} identified two genuine points of contact between the localized
...
the group the proposal computed.} By Proposition 2.2 of Chapter~\ref{ch:II}, the isotropy
...
Proposition 5.3 of Chapter~\ref{ch:XXII}.
...
coordinate projection. The framework is that of [Ch.\begin{proposition}[Kernel and projection ranks]\label{XXIII:prop:rank}
The group $\overline{\cO^\times_{K,+}}\cap\prod_{\pp\in T}\cO_\pp^\times=\ker\pi_{T^c}$
satisfies
\begin{equation}\label{XXIII:eq:rank}
...
which is \eqref{XXIII:eq:rank}.
\end{proof}

\begin{theorem}[The isotropy, identified; Leopoldt does not give triviality]\label{XXIII:thm:nottrivial}
...
$\alpha_a$. By Proposition 2.2 of Chapter~\ref{ch:II},
...
Chapter~\ref{ch:II}'s unconditional case;
\item if $r\ge1$ and $d_{T^c}=1$, the image of $\pi_{T^c}$ is \emph{open} in the
one-dimensional $\cO_{\pp'}^\times$ (Proposition~\ref{XXIII:prop:rank}, last part: a nonzero
...
\begin{remark}[Numerical table]\label{XXIII:rem:table}
With $p$ split completely and $|T^c|=1$, so $|T|=g-1=n-1$ and $d_{T^c}=1$, the corrected
isotropy rank is:
\[
\begin{array}{lcccc}
K & r & |T| & \text{kernel rank \eqref{XXIII:eq:rank}} & \rank\Iso(Y_T)\\\hline
\Q & 0 & 0 & 0 & 0\\
...
\end{array}
\]
The last column is Theorem~\ref{XXIII:thm:nottrivial}(2) (full rank $|T|$ as soon as $r\ge1$),
...
precisely those in which Chapter~\ref{ch:II} (its Proposition 2.2 and Remark 2.3) is unconditional.
\end{remark}

\begin{remark}[Nothing in the classification depends on this]\label{XXIII:rem:nodependence}
The isotropy on the strata does not enter the main results. Chapter~\ref{ch:II}
...
Theorem~\ref{XXIII:thm:nottrivial} corrects a proposed characterization of Leopoldt without
disturbing anything downstream.
\end{remark}

\begin{remark}[The residual condition, and where Leopoldt sits]\label{XXIII:rem:relative}
For $d_{T^c}\ge2$ the finiteness of the quotient in Theorem~\ref{XXIII:thm:nottrivial} requires
...
\begin{proposition}[Order of vanishing and leading term]\label{XXIII:prop:euler}
...
\begin{equation}\label{XXIII:eq:leading}
...
\begin{theorem}[The proposed $\cL$-invariant vanishes]\label{XXIII:thm:Lzero}
...
\begin{remark}[Real versus $p$-adic]\label{XXIII:rem:realvsp}
The confusion is between two logarithms. Formula \eqref{XXIII:eq:leading} is a computation with the
...
Proposition 2.1 of Chapter~\ref{ch:XXII}, that $\Ns\pp$ is not a $p$-adic unit.
\end{remark}

\begin{remark}[What is right about the analogy]\label{XXIII:rem:rightpart}
...
and it is the one supported by Proposition 5.1 of Chapter~\ref{ch:XXII}: the $p$-local partition function
...
\begin{proposition}[The volume statement is not well formed]\label{XXIII:prop:novolume}
...
\begin{theorem}[The correct statement]\label{XXIII:thm:dimension}
\[
...
\begin{example}\label{XXIII:ex:dim}
...
\begin{corollary}[Agreement with Chapter~\ref{ch:XXII}]\label{XXIII:cor:agree}
Theorem~\ref{XXIII:thm:dimension} is the computation
\[
\rank_{\Z_p}\Gal(M_\infty/K)=r_2+1+\delta_p(K)
\]
of Chapter~\ref{ch:XXII} (its Proposition 5.3), transported through class field theory:
...
The two bridges identified in Chapter~\ref{ch:XXII} are real, and both survive; what fails is the
...
That last point is worth isolating, because it recurs. In Proposition 2.1 of Chapter~\ref{ch:XXII} the
...
Proposition~\ref{XXIII:prop:rank} every projection rank is budgeted by $r-\delta_p(K)$, some
aggregate of the image codimensions does see $\delta_p$ and might work. And, given Remark~\ref{XXIII:rem:rightpart}, is the correspondence
