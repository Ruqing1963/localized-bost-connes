% generated from papers/II-structure/paper_II_structure.tex -- edit the paper, then rerun assemble.py
\chapter[Ideals, boundary, and index]{Ideal structure, boundary quotient, and index}\label{ch:II}
\chapterorigin{This chapter is Paper II of the series (\texttt{papers/II-structure/}).}
\begin{summary}
...
We keep the notation of [Ch.~\ref{ch:Main}]: $K$ is a number field, $S$ a set of finite primes,
...
\]
Recall the exact sequence [Ch.~\ref{ch:Main}, (2.1)]
\begin{equation}\label{II:eq:exact}
1\to U_S/\overline{\cO^\times_{K,+}}\to G_S\to\Cl^+_S\to1,\qquad h^+_S:=|\Cl^+_S| .
\end{equation}

\begin{lemma}[Single isometry, nested projections]\label{II:lem:single}
...
infinite --- follows from Theorem~\ref{II:thm:boundary} below: $\cA_{K,S}$ maps unitally onto
...
Lemma~\ref{II:lem:single} is the source of every structural statement below. We flag its use
...
additive cosets $a+\aaa$, whose ranges \emph{partition}; \S\ref{II:sec:affine} restores that
ingredient and confirms that it is the cause.

\section{Isotropy, topological principality, and the ideal lattice}

Let $\cG_S=I_S\ltimes\widetilde Y_S$ be the dilated groupoid of [Ch.~\ref{ch:Main}, Prop.~2.8],
with points $[x,g]$ subject to $(wx,g)\sim(x,r(w)g)$ for $w\in U_S$.

\begin{lemma}[Isotropy formula]\label{II:lem:iso}
For $y=[x,g]$ put $T(y)=\{\pp\in S:x_\pp=0\}$. Then
\begin{equation}\label{II:eq:iso}
...
\begin{proposition}[Isotropy off the boundary]\label{II:prop:nojump}
Write $\Idl_T=\{u\in\Idl_S:\ u_{\pp'}=1\ \text{for all }\pp'\in S\setminus T\}$. Then for
$y$ with $T(y)=T$,
\begin{equation}\label{II:eq:isodiv}
...
\begin{proof}
By Lemma~\ref{II:lem:iso}, $\aaa\in I_T$ lies in the isotropy iff $\sigma_\aaa\in r(U_T)$, i.e.\
...
is trivial at every $\pp'\notin T$ and has $\mathrm{div}(u)=\aaa$, which is \eqref{II:eq:isodiv};
...
the fibre is $G_S/r(U_S)\cong\Cl^+_S$ by \eqref{II:eq:exact}, and \eqref{II:eq:iso} with $T=S$
becomes $\{\aaa:[\aaa]=1\}=I_S^0$.
\end{proof}

\begin{remark}[The general case, and why it does not matter here]\label{II:rem:leopoldt}
When $\cO^\times_{K,+}$ is infinite the right-hand side of \eqref{II:eq:isodiv} can in
...
Theorem~\ref{II:thm:principal} needs only that the strata are nowhere dense, which holds
whatever their isotropy, since for infinite $S$ every basic clopen set contains points with
$x_\pp\ne0$ at any prescribed $\pp$.
\end{remark}

\begin{remark}\label{II:rem:whynojump}
...
\begin{theorem}[Topological principality, and the two halves of the ideal lattice]\label{II:thm:principal}
...
Proposition~\ref{II:prop:nojump}, whenever $\cO^\times_{K,+}$ is finite --- then the ideals of
...
\begin{proof}
(1) By \eqref{II:eq:iso} every unit with nontrivial isotropy lies in some stratum
...
Proposition~\ref{II:prop:nojump}.) The restriction $\cG_S|_{Y_S}$ is then topologically
...
$X=\widehat{I_S^0}$ by Theorem~\ref{II:thm:boundary}; its ideals are the $M_h(C_0(X\setminus Z))$,
...
\begin{remark}\label{II:rem:lattice}
...
infinite, Remark~\ref{II:rem:leopoldt} leaves open whether some proper stratum has nontrivial
isotropy; if it does, that stratum contributes further ideals of the same kind.
\end{remark}

\begin{proposition}[The lattice is not the power set of $S$]\label{II:prop:lattice}
...
\begin{equation}\label{II:eq:oneprime}
...
with quotient $\Z_\ell^\times$, giving the ideal in \eqref{II:eq:oneprime}; the fixed point
...
ideal, and the quotient $C(\T)$ in \eqref{II:eq:oneprime} is the $h^+_S=1$, $I_S^0=\Z$ case of
Theorem~\ref{II:thm:principal}(2).
\end{remark}

\section{The boundary quotient}

\begin{theorem}[The tight quotient is abelian-by-finite]\label{II:thm:boundary}
...
$C^*(I_S\ltimes\partial Y_S)=C(\Cl^+_S)\rtimes I_S$ by Proposition~\ref{II:prop:nojump}. In the
...
\begin{remark}[Why no Cuntz relation is available]\label{II:rem:nocuntz}
By Lemma~\ref{II:lem:single}(2) the projections $\mathbf 1_{\aaa Y_S}$ are nested. Imposing
...
unitarity, and this is what Theorem~\ref{II:thm:boundary} implements. The Cuntz alternative
requires a partition; see \S\ref{II:sec:affine}.
\end{remark}

\begin{theorem}[The class of the unit records $h^+_S$]\label{II:thm:Kboundary}
...
\begin{equation}\label{II:eq:unit}
...
\begin{theorem}[Rigidity of the boundary]\label{II:thm:rigid}
...
apply Theorem~\ref{II:thm:Kboundary}. For the last statement, $C(\Cl^+_S)\rtimes I_S$ depends
...
\begin{lemma}\label{II:lem:cantorspace}
...
For $S$ finite with $|S|\ge2$ the primes interact: by [Ch.~\ref{ch:Main}, Lem.~2.10] the generator
...
\begin{proposition}[Finite $S$]\label{II:prop:Kfinite}
...
[Ch.~\ref{ch:Main}, Prop.~2.7] (see Remark~\ref{II:rem:embed} below).
\end{proposition}
...
$K_*(\cA_{K,S})$ beyond Propositions \ref{II:prop:K} and \ref{II:prop:Kfinite}; the top group
$H_{|S|}$ is the group of $\Z^S$-invariant compactly supported functions, which vanishes by
the argument of Proposition~\ref{II:prop:K}, but the intermediate groups receive contributions
from the strata $\{x_\pp=0,\ \pp\in T\}$, on which $\Z^T$ acts on $G_S$ by translations
with nontrivial invariants.

\begin{example}[$K=\Q$, $S=\{\ell\}$]\label{II:ex:oneprime}
Here $G_S=\Z_\ell^\times$ and Proposition~\ref{II:prop:K} gives $K_1(\cA)=0$ and
$K_0(\cA)\cong C(\Z_\ell^\times,\Z)/\Z\oplus\Z\cong\Z^{(\infty)}$, in agreement with
\eqref{II:eq:oneprime}. A direct computation of the Smith normal form of $1-\alpha_*$ on
level-$n$ functions, for $\ell=2,3,5$ and $n\le3$, gives kernel $0$ and free cokernel of
rank $\ell^{n+1}-\ell^n$, with no torsion.
\end{example}

\begin{remark}[$K$-theory carries no arithmetic: the mechanism]\label{II:rem:noarith}
...
is structural rather than accidental: by Propositions \ref{II:prop:K} and \ref{II:prop:Kfinite}
...
$K_0$ annihilates precisely the data of interest. This is taken up in Chapter~\ref{ch:III} of the
series.
\end{remark}

\section{Index theory}

\begin{remark}[Nested prime sets]\label{II:rem:embed}
For $S_1\subset S_2$ the embedding $\cA_{K,S_1}\hookrightarrow\cA_{K,S_2}$, $f\mapsto f\circ\mathrm{pr}$,
$\mu_\aaa\mapsto\mu_\aaa$, of [Ch.~\ref{ch:Main}, Prop.~2.7] requires an $I_{S_1}$-equivariant
continuous surjection $\mathrm{pr}:Y_{S_2}\to Y_{S_1}$. By \eqref{II:eq:exact}, $Y_S$ is a
...
[Ch.~\ref{ch:Main}, Prop.~2.7] must be read with this qualification (the uses made of that
proposition in [Ch.~\ref{ch:Main}] are over $\Q$). \emph{Throughout this section we assume
...
$\cA_{K,S_1}\subseteq\cA_{K,S_2}$ via Remark~\ref{II:rem:embed}.

\begin{proposition}[The conditional expectation exists]\label{II:prop:E}
...
\begin{theorem}[Prime inclusions have infinite index]\label{II:thm:infinite}
...
coordinates of [Ch.~\ref{ch:Main}, Lem.~2.10] its measure is $\nu=\pi_\beta\otimes\mathrm{Haar}$.
...
$\varphi_2(\mathbf 1_{\pp^kY})=\Ns\pp^{-k\beta}$ [Ch.~\ref{ch:Main}, Prop.~2.8], and
...
[Ch.~\ref{ch:Main}, Prop.~6.1]. Fix $k,j\ge0$ and let $T:\{v_\pp=k\}\to\{v_\pp=j\}$,
...
\begin{remark}[Scope]\label{II:rem:scope}
Part (1) holds for every KMS state; part (2) uses the product form of $\nu$ only to build
$W$. For a non-symmetric extremal state the fibre measure $\rho_v$ of [Ch.~\ref{ch:Main}, Lem.~2.10]
...
\begin{remark}[Why no index formula can exist]\label{II:rem:noformula}
...
Theorem~\ref{II:thm:Kboundary} and in \eqref{II:eq:exact} --- but not in an index.
\end{remark}

\begin{proposition}[No Galois correspondence with prime sets]\label{II:prop:nogalois}
In the situation of Theorem~\ref{II:thm:infinite}(2) let $S_2=S_1\cup\{\pp\}$, so there are
...
of Theorem~\ref{II:thm:infinite} gives $c_je_j=me_j=W(me_k)W^*=c_ke_j$ for $j,k\in B$, so
...
\begin{lemma}[Central decomposition]\label{II:lem:central}
\begin{equation}\label{II:eq:central}
...
\begin{proof}
By [Ch.~\ref{ch:Main}, Thm.~3.6] the $\mathrm{KMS}_\beta$ simplex is $\Prob(G_S/\Xi_\beta^\perp)$,
...
$\pi_{\phisym}=\bigoplus_\varepsilon\pi_{\varphi_\varepsilon}$, giving \eqref{II:eq:central},
the centre, and $\omega_{\phisym}(z_\varepsilon)=|G|^{-1}$. By [Ch.~\ref{ch:Main}, Thm.~3.6(3)] the
...
\begin{theorem}[The finite-index inclusion]\label{II:thm:finite}
...
\begin{proof}
$G$ permutes the summands of \eqref{II:eq:central} simply transitively, so an invariant element
is determined by one component and $\Msym^G\cong D$ embedded diagonally via the $\theta_g$,
which is \eqref{II:eq:trivialincl}. In \eqref{II:eq:trivialincl} the averaging map is
...
\begin{remark}[Not a subfactor invariant]\label{II:rem:nostandard}
The inclusion \eqref{II:eq:trivialincl} is the trivial inclusion of the factor $D$ tensored
...
and in the way $G_S$ permutes the summands; this is what Corollary~\ref{II:cor:staircase}
quantifies. Along a cascade with $\Xi_\beta\cong(\Z/2\Z)^{A_\beta}$ [Ch.~\ref{ch:Main}, Prop.~4.16],
crossing a threshold $\beta_n$ doubles the number of summands and doubles the index.
\end{remark}

\begin{lemma}[Relative entropy of the phases]\label{II:lem:relent}
With the notation of Lemma~\ref{II:lem:central}, and $\Ent$ the relative entropy,
\[
...
this to \eqref{II:eq:central} with $\lambda=\delta_\varepsilon$ and $\mu_\delta=|G|^{-1}$ for
every $\delta$, which is Lemma~\ref{II:lem:central}. On the summand $\varepsilon$ the two
...
\begin{corollary}[Index staircase equals information staircase]\label{II:cor:staircase}
\[
\log\bigl[\Msym:\Msym^G\bigr]_0=|A_\beta|\log2=\Ent\bigl(\omega_{\varphi_\varepsilon}\big\|\,\omega_{\phisym}\bigr),
\]
by \eqref{II:eq:index} and Lemma~\ref{II:lem:relent}. The subfactor index and the relative
...
by \eqref{II:eq:trivialincl}; and the relative entropy of one extremal state against the
...
Lemma~\ref{II:lem:relent}, where the relative entropy is shown to be purely central.
\end{corollary}

\section{The control experiment: restoring the additive part}\label{II:sec:affine}

To confirm that Lemma~\ref{II:lem:single} is the cause of \S\S2--5, we restore the missing
ingredient.

\begin{remark}[The additive group is $\cO_K$]\label{II:rem:additive}
...
\begin{lemma}[Finite covers]\label{II:lem:cover}
...
\begin{equation}\label{II:eq:cuntz}
...
than the nested inequality of Lemma~\ref{II:lem:single}; and \eqref{II:eq:cuntz} already holds in
$\cA^{\mathrm{aff}}_{K,S}$, since the Stacey covariance gives
$\mu_{(a,\pp)}\mu_{(a,\pp)}^*=\mathbf 1_{a+\pp\hO_S}$.
\end{proof}

\begin{theorem}[Kirchberg boundary]\label{II:thm:kirchberg}
...
\begin{theorem}[The unit class says little even here]\label{II:thm:unit}
...
\begin{proof}
Each $\mu_{(a,\pp)}$ is an isometry, so $[\mu\mu^*]=[1]$; applying $K_0$ to \eqref{II:eq:cuntz}
gives $[1]=\Ns\pp[1]$.
\end{proof}

\begin{proposition}[The Cuntz relation pins the temperature]\label{II:prop:betaone}
...
\eqref{II:eq:cuntz}, the KMS condition gives
$\varphi(\mu\mu^*)=\varphi(\mu^*\sigma_{i\beta}(\mu))=\Ns\pp^{-\beta}\varphi(\mu^*\mu)=\Ns\pp^{-\beta}$;
summing \eqref{II:eq:cuntz} gives $1=\Ns\pp^{1-\beta}$, so $\beta=1$.
\end{proof}
...
\begin{remark}[Two behaviours of the unit class]\label{II:rem:twounits}
Theorems \ref{II:thm:Kboundary} and \ref{II:thm:unit} exhibit opposite behaviours of $[1]$, and
...
projections of Lemma~\ref{II:lem:single} are an advantage: they leave $K_0$ torsion-free, and a
...
Everything in the left column follows from Lemma~\ref{II:lem:single}, and the right column
...
system (Proposition~\ref{II:prop:betaone}), whereas the multiplicative system has
$\mathrm{KMS}_\beta$ states for every $\beta>0$ [Ch.~\ref{ch:Main}, Thm.~3.6] --- and the free
variant treated in Chapter~\ref{ch:V}, where orthogonal ranges again restrict $\beta$.

Four questions remain. We have computed $K_*(\cA_{K,S})$ only for a single prime, and
$K_*(\cA^{\mathrm{aff}}_{K,S})$ not at all; by Theorem~\ref{II:thm:kirchberg} the triple
$(K_0,[1],K_1)$ is a complete invariant of the affine algebra and by Theorem~\ref{II:thm:unit}
...
(Remark~\ref{II:rem:leopoldt}) --- a Leopoldt-type closure question --- and hence contributes
further ideals of the kind seen in Theorem~\ref{II:thm:principal}(2), is open. Whether the
infinite index of Theorem~\ref{II:thm:infinite} persists at the non-symmetric extremal states
(Remark~\ref{II:rem:scope}), and whether these inclusions are discrete in the sense of
...
$\widehat{\Xi_\beta}$ (Theorem~\ref{II:thm:finite}), which remembers $\Xi_\beta$ only as a finite
set; it distinguishes neither $K$ nor $S$ nor the thresholds, and the question of which
invariant does is taken up in Chapter~\ref{ch:III}.

It is worth saying plainly why the blindness recorded in Remark~\ref{II:rem:noarith} is not a
computational accident. By Propositions \ref{II:prop:K} and \ref{II:prop:Kfinite} the passage
...
[Ch.~\ref{ch:Main}, Prop.~2.8] are eigenvectors,
