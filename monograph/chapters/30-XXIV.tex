% generated from papers/XXIV-sieve/paper_XXIV_sieve.tex -- edit the paper, then rerun assemble.py
\chapter[Sieve primes and their phases]{The statistical mechanics of sieve-theoretic primes:\\ the critical spectrum, the log-power dichotomy,\\ and two families that are never unique}\label{ch:XXIV}
\chapterorigin{This chapter is Paper XXIV of the series (\texttt{papers/XXIV-sieve/}).}
\begin{summary}
We compute the thermodynamics of the classical sieve-theoretic prime families for the
localized Bost--Connes systems of [Ch.~\ref{ch:Main}]. The critical exponents proposed are all
...
obstruction in the sense of [Ch.~\ref{ch:Main}, \S4.1], and \textbf{for these two families the
...
generates $(\Z/3)^\times$, and by [Ch.~\ref{ch:Main}, Lem.~5.6] the admissible classes generate
...
The finding is consistent with [Ch.~\ref{ch:Main}, \S5]: Theorem 5.5(5) there, with Lemma 5.6,
...
The framework and notation are those of [Ch.\begin{proposition}[Abel summation]\label{XXIV:prop:abel}
Let $S\subseteq P$ satisfy $\pi_S(x)\asymp x^\theta/\log^kx$ with $\theta\in(0,1]$, $k\ge0$.
Then
\begin{equation}\label{XXIV:eq:abel}
...
\begin{theorem}[The spectrum]\label{XXIV:thm:spectrum}
\[
\begin{array}{lccll}
...
\begin{proof}
Proposition~\ref{XXIV:prop:abel} with the counting functions: Brun's sieve \cite{HR} gives the upper bounds
...
\begin{remark}[The dichotomy is the second prime condition]\label{XXIV:rem:dichotomy}
By Proposition~\ref{XXIV:prop:abel} the phase is closed exactly when $k>1$, and $k=2$ occurs
...
[Ch.~\ref{ch:Main}, Thm.~5.5] is not a feature of the sieve but of the second condition, and it
...
\begin{lemma}[Elementary congruences]\label{XXIV:lem:congruence}
\begin{enumerate}
...
\begin{theorem}[Permanent obstruction --- and its exact scope]\label{XXIV:thm:permanent}
...
convention of [Ch.~\ref{ch:Main}, (1.1)] inverting $\chi(p)$ without changing the modulus of the
summand. Then:
\begin{enumerate}
...
Lemma~\ref{XXIV:lem:congruence}(3)--(4) all odd members lie in the class $1\bmod4$, which is
...
effect: by Lemma~\ref{XXIV:lem:congruence}(1)--(2) all members $p>3$ lie in the class $2\bmod3$,
...
[Ch.~\ref{ch:Main}, Lem.~5.6] the admissible classes of these families generate
...
[Ch.~\ref{ch:Main}, Lem.~5.6]. The never-uniqueness in (1) is [Ch.~\ref{ch:Main}, Thm.~3.9], uniqueness
requiring $\sum_{D_\chi}p^{-\beta}=\infty$ for every nontrivial $\chi$; the obstruction is
permanent in the sense of [Ch.~\ref{ch:Main}, \S4.1].
\end{proof}

\begin{remark}[Numerical verification]\label{XXIV:rem:numver}
...
\end{array}
\]
The single-class rows now illustrate \emph{both} halves of Theorem~\ref{XXIV:thm:permanent}:
...
\begin{corollary}[The proposed criterion is not the right one]\label{XXIV:cor:criterion}
...
[Ch.~\ref{ch:Main}, Thm.~5.5(5)]: confinement to a proper \emph{kernel}. That happens for Landau
and Friedlander--Iwaniec, and for no other family here.
\end{corollary}

\begin{remark}[Consequence for [Ch.~\ref{ch:Main}]]\label{XXIV:rem:consequence}
[Ch.~\ref{ch:Main}, Thm.~5.5] establishes for the Sophie Germain family the Brun-summability, the
closed low-temperature phase, and --- in its part (5), with [Ch.~\ref{ch:Main}, Lem.~5.6] --- the
uniqueness criterion in the divergent range. All of it stands, and
Theorem~\ref{XXIV:thm:permanent}(2) is its confirmation from the congruence side: the mod-$3$
...
the series diverges (conditional on the lower bound of [Ch.~\ref{ch:Main}, Thm.~5.5(2)]); and full
...
\begin{theorem}[Piatetski-Shapiro]\label{XXIV:thm:PS}
...
\begin{remark}[The range is narrow]\label{XXIV:rem:narrow}
...
$\mathrm{KMS}$ states are never unique by [Ch.~\ref{ch:Main}, Thm.~3.9]; for twin and Sophie
