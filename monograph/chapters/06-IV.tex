% generated from papers/IV-metric/paper_IV_metric.tex -- edit the paper, then rerun assemble.py
\chapter[Noncommutative metric geometry]{Noncommutative metric geometry}\label{ch:IV}
\chapterorigin{This chapter is Paper IV of the series (\texttt{papers/IV-metric/}).}
\begin{summary}
Three invariants of the localized Bost--Connes system jump by a fixed amount at each phase
transition: the rank of the obstruction group jumps by one [Ch.~\ref{ch:I}], the
subfactor index doubles, and the relative entropy of an extremal phase against the
symmetric one increases by $\log2$ (Chapters~\ref{ch:II} and \ref{ch:III}). We show that the
...
We use the notation of [Ch.~\ref{ch:Main}]. In the symmetry-broken phase of [Ch.~\ref{ch:Main}, \S4.2] ---
...
[Ch.~\ref{ch:I}, Thm.~6.5]; the index $[\,\Msym:M_\varepsilon]$ of the symmetry-breaking
inclusion doubles (Chapter~\ref{ch:II}); and the relative entropy
$\Ent(\omega_{\varphi_\varepsilon}\|\omega_{\phisym})$ increases by $\log2$ (Chapter~\ref{ch:III}).
All three count phases, and phases are counted in integers.

The metric invariant behaves differently, and Theorem~\ref{IV:thm:continuous} is the reason for
this paper. The mechanism is Theorem~\ref{IV:thm:notlip}: $\Psi$ cannot be Lipschitz for a
...
\begin{proposition}[The modular direction is metrically blind]\label{IV:prop:blind}
...
\begin{remark}[Why we work on the base]\label{IV:prop:reduction}
Proposition~\ref{IV:prop:blind} says that a metric on the $\mathrm{KMS}_\beta$ simplex must
...
$Y_S$ and the canonical expectation $E$ [Ch.~\ref{ch:Main}, Prop.~2.8], and $\varphi\mapsto\nu$ is an
...
\]
with $L$ the seminorm of Lemma~\ref{IV:lem:L}. If a Dirac operator $D$ on a representation of
...
operator of Definition~\ref{IV:def:D} does not have this property: it must weight the level
$k$ of each coordinate $v_\ell$ by a summable sequence $w_{\ell,k}$ (Theorem~\ref{IV:thm:cqms}),
...
\begin{equation}\label{IV:eq:X}
X:=G_S\times\overline{\N}^{\,S},\qquad\overline\N=\N\cup\{\infty\},
\end{equation}
compact metrizable and totally disconnected, on which by [Ch.~\ref{ch:Main}, Lem.~2.10]
\begin{equation}\label{IV:eq:nusym}
...
\begin{definition}[Moves and weights]\label{IV:def:moves}
...
\begin{equation}\label{IV:eq:summable}
...
\begin{definition}\label{IV:def:D}
...
\begin{lemma}[Commutator formula]\label{IV:lem:L}
For $f\in C(X)$,
\begin{equation}\label{IV:eq:L}
...
\begin{lemma}[Kernel and paths]\label{IV:lem:kernelpath}
...
\begin{theorem}[Compact quantum metric space]\label{IV:thm:cqms}
Under \eqref{IV:eq:summable}, $(C(X),L)$ is a compact quantum metric space in the sense of
Rieffel \cite{Rieffel98,Rieffel}: $L$ is a lower semicontinuous Lip-norm with kernel
...
Lemma~\ref{IV:lem:kernelpath} gives $\|f\|_\infty\le\Sigma$ and equicontinuity: the sets
...
$X$ is compact, so Arzel\`a--Ascoli applies. Lower semicontinuity follows from \eqref{IV:eq:L}, each
$f\mapsto\Delta_jf$ being continuous for uniform convergence.
\end{proof}

\begin{remark}[Summability is necessary]\label{IV:rem:necessary}
...
\begin{lemma}[Elementary differences]\label{IV:lem:diffPsi}
...
\begin{theorem}[Exclusivity]\label{IV:thm:notlip}
Assume \eqref{IV:eq:summable} and $R\ne\emptyset$. Then $L(\Psi)=\infty$, and indeed
$L(\Psi_N)=\infty$ for every $N\ge\min R$, where
$\Psi_N=\chi_3\prod_{\ell\in R,\ \ell\le N}(-1)^{v_\ell}$, $L$ being extended to bounded
Borel functions by the same formula \eqref{IV:eq:L}. No truncation of the order parameter
beyond the level of $\chi_3$ lies in the Lipschitz domain of the operators of
Definition~\ref{IV:def:D}; and since $\Psi_N$ is discontinuous along $\{v_\ell=\infty\}$ for
$\ell\in R$, $\ell\le N$, it lies in the Lipschitz domain of no Lip-norm on $C(X)$
whatever.
\end{theorem}

\begin{proof}
By Lemma~\ref{IV:lem:diffPsi}, at a point $x$ with $v_\ell(x)=k$,
\[
...
\eqref{IV:eq:L} is $+\infty$. The same computation truncated at $N$ gives the second assertion.
\end{proof}

\begin{corollary}[Quantitative divergence]\label{IV:cor:quant}
...
\begin{equation}\label{IV:eq:quant}
...
\begin{remark}[Interpretation]\label{IV:rem:impedance}
...
\begin{lemma}[Reduction to odd functions]\label{IV:lem:odd}
...
\begin{theorem}[Squeeze]\label{IV:thm:squeeze}
\begin{equation}\label{IV:eq:squeeze}
w_0\,\Pi_\infty(\beta)\ \le\ W_1(\varphi_+,\varphi_-)\ \le\ w_0 .
\end{equation}
...
\begin{proof}
By Remark~\ref{IV:prop:reduction} and \eqref{IV:eq:Psi},
...
Lemma~\ref{IV:lem:diffPsi} its only nonzero elementary difference is $\Delta_0f=-w_0\chi_3$, so
$L(f)=w_0^{-1}w_0=1$; and $\chi_3$ and $\sigma$ being independent under \eqref{IV:eq:nusym},
\[
2\langle f,\Psi\rangle=w_0\langle\chi_3,\chi_3\sigma\rangle=w_0\,\mathbb E[\sigma]=w_0\Pi_\infty .
\]

\emph{Upper bound.} By Lemma~\ref{IV:lem:odd} take $f=\chi_3h$ odd with $L(f)\le1$. Then
$|\Delta_0f|=2|h|$ pointwise, so \eqref{IV:eq:L} gives $2w_0^{-1}\|h\|_\infty\le1$; since
$|\sigma|=1$,
...
$\nu_+=\nu_-$ and $W_1=0$; otherwise $\Pi_\infty>0$ by Lemma~\ref{IV:lem:kakutani}.
\end{proof}

\begin{remark}[Locality versus tail behaviour]\label{IV:rem:locality}
...
\begin{lemma}[Poincar\'e inequality for the geometric law]\label{IV:lem:poincare}
For $0<t<1$, $\mu_t(k)=(1-t)t^k$ and $\Delta h(k)=h(k+1)-h(k)$,
\begin{equation}\label{IV:eq:poincare}
...
\begin{theorem}[Continuous vanishing at $\beta_c$]\label{IV:thm:continuous}
Assume the weights satisfy \eqref{IV:eq:summable} and are bounded, let $\beta_c=\beta_c(R)>0$
...
the cascade sets of [Ch.~\ref{ch:Main}, Prop.~4.16] and [Ch.~\ref{ch:I}, Lem.~5.4]. Then
\[
...
Theorem~\ref{IV:thm:squeeze} gives $W_1\ge w_0\Pi_\infty(\beta_c)>0$ there, while $W_1=0$ for
...
Theorem~\ref{IV:thm:squeeze}. Let $\mathcal F_N=\sigma(u;v_\ell,\ \ell\le N)$ and $E_N$ the
...
and by Lemma~\ref{IV:lem:poincare},
...
$\Pi_N(\beta)\to0$ by Lemma~\ref{IV:lem:kakutani} and $w_0\Pi_N(\beta)<\varepsilon/2$ for
$\beta$ close enough to $\beta_c$.
\end{proof}

\begin{remark}[First order versus second order]\label{IV:rem:orders}
\[
\begin{array}{lcc}
& \beta\le\beta_c & \beta>\beta_c\\\hline
\text{number of extreme }\mathrm{KMS}_\beta\text{ states} & 1 & 2\quad(\text{jump})\\
\text{rank of }\Xi_\beta\ \text{[Ch.~\ref{ch:I}]} & 0 & 1\quad(\text{jump})\\
\text{index }[\Msym:M_\varepsilon]\ \text{(Chapter~\ref{ch:II})} & 1 & 2\quad(\text{jump})\\
\Ent(\omega_{\varphi_\pm}\|\omega_{\phisym})\ \text{(Chapter~\ref{ch:III})} & 0 & \log2\quad(\text{jump})\\
...
Theorem~\ref{IV:thm:notlip} is why: were $\Psi$ Lipschitz, the test function $\Psi/L(\Psi)$
...
\begin{remark}[A model computation]\label{IV:rem:numerics}
For $R=\{r\}$ a single prime and constant weights $w_0$, $w_r$ along the $r$-line ---
for which \eqref{IV:eq:summable} fails and $C(X)$ is not a compact quantum metric space, but
the reduction of Lemma~\ref{IV:lem:odd} still applies and $L(f)=\sup_k\bigl(4w_0^{-2}h(k)^2+w_r^{-2}|h(k+1)-h(k)|^2\bigr)^{1/2}$
...
constant $h\equiv w_0/2$, which is the lower bound $w_0m_r$ of \eqref{IV:eq:squeeze}, and the
...
squeeze \eqref{IV:eq:squeeze} holds in every case, and its lower bound is far from sharp for
$t$ near $1$. This model has $\sum_{\ell\in R}\ell^{-\beta}=t<\infty$ for all $\beta$, hence
no transition, and cannot probe Theorem~\ref{IV:thm:continuous}: the collapse there requires
$R$ infinite and is driven by $\rho_N\to0$, which has no analogue for $|R|=1$. We record
the table only as a check on \eqref{IV:eq:squeeze}. In Lemma~\ref{IV:lem:poincare} the quantity
...
exact value of $W_1(\varphi_+,\varphi_-)$ is unknown: Theorem~\ref{IV:thm:squeeze} gives
$w_0\Pi_\infty\le W_1\le w_0$ and Remark~\ref{IV:rem:numerics} shows both bounds can be far
from the truth; the upper bound $w_0\Pi_N+2\rho_N$ from the proof of
Theorem~\ref{IV:thm:continuous} suggests the answer is $\inf_N(w_0\Pi_N+2\rho_N)$ up to
...
representation of $\cA_{K,S}$, and Remark~\ref{IV:prop:reduction} explains why the summable
level weights that Theorem~\ref{IV:thm:cqms} requires are incompatible with bounded
commutators with the isometries, while by Proposition~\ref{IV:prop:blind} the modular operator
...
Third, for the cascade systems of [Ch.~\ref{ch:I}] each generator $\chi_n$ has its own
elementary move and the analogue of Theorem~\ref{IV:thm:squeeze} should give a family of bounds
