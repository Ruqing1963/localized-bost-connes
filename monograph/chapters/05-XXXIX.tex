% generated from papers/XXXIX-cohomology/paper_XXXIX_cohomology.tex -- edit the paper, then rerun assemble.py
\chapter[Measurable cohomology of the tail relation]{The measurable first cohomology of the tail relation:\\ turbulence and the arithmetic embedding}\label{ch:XXXIX}
\renewcommand{\R}{\mathcal{R}}
\chapterorigin{This chapter is Paper XXXIX of the series (\texttt{papers/XXXIX-cohomology/}).}
\begin{summary}
Chapter~\ref{ch:III} identified the obstruction group $\Xi_\beta(K,S)$ as the kernel of the natural
...
\section{The structure theorem: what can and cannot be said}\label{XXXIX:sec:structure}
...
\begin{theorem}\label{XXXIX:thm:structure}
...
\S\ref{XXXIX:sec:assessment}.
\end{proof}

\begin{proposition}[The smooth residues, completely]\label{XXXIX:prop:residues}
...
\section{The arithmetic embedding, and the amnesia of the ambient group}\label{XXXIX:sec:embedding}

\begin{theorem}\label{XXXIX:thm:embed}
The map $\GS\to\Hmeas$, $\chi\mapsto[c_\chi]$, of Chapter~\ref{ch:III} is a group homomorphism
...
in Chapter~\ref{ch:I}. The ambient cohomology is amnesiac; the arithmetic of $(K,S,\beta)$ survives
exactly as the embedded subgroup $\GS/\Xi_\beta$ and its position inside the turbulence.
\end{theorem}

\begin{proof}
Multiplicativity and the kernel identification are Chapter~\ref{ch:III}'s Theorem 2.3, restated;
...
\section{The verdict}\label{XXXIX:sec:verdict}
...
spectral distance and entropy are smooth instruments, and Theorem~\ref{XXXIX:thm:structure}
...
\section{Assessment}\label{XXXIX:sec:assessment}
