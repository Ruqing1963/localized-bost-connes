...
\emph{(i) Separating $q$ from $h$:} the abstract K-groups see only the composite $q^h$;
the candidate for separating the pair is the dual coaction (the gauge circle of the
repaired system has minimal period $h$ times that of a principal prime of the same norm),
not the K-groups alone. \emph{(ii) Galois descent from the Hilbert class field:}
