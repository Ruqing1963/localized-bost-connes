% generated from papers/VII-galois-groupoid/paper_VII_galois_groupoid.tex -- edit the paper, then rerun assemble.py
\chapter[The Galois-twisted groupoid]{The Galois-twisted arithmetic groupoid:\\ its KMS classification, and why the Tannakian obstruction\\ does not compute it}\label{ch:VII}
\chapterorigin{This chapter is Paper VII of the series (\texttt{papers/VII-galois-groupoid/}).}
\begin{summary}
...
This is \emph{not} $\Prob(G/N_\beta)$, with $N_\beta$ the Tannakian subgroup of Chapter~\ref{ch:VI}
...
genuinely non-abelian theory needs a non-abelian acting semigroup, which by Chapter~\ref{ch:V} of the
...
[Ch.~\ref{ch:Main}, Lem.~2.10]; $\pi_\beta$ charges no hyperplane $\{v_\pp=\infty\}$. Let $\cR_S$ be the
tail equivalence relation on $V$, which is ergodic for $\pi_\beta$ by the Kolmogorov
zero-one law [Ch.~\ref{ch:Main}, Lem.~3.1]. The semigroup $J_S=\N^{(S)}$ acts on $X$ by
...
\begin{definition}\label{VII:def:groupoid}
...
as $\cA_{K,S}=C(Y_S)\rtimes J_S$ sits in the dilated groupoid algebra in [Ch.~\ref{ch:Main}, \S2]; the
...
\begin{lemma}[Basic structure]\label{VII:lem:basic}
\begin{enumerate}
...
\item $\cG_{G,S}$ is second countable, étale and amenable, so $C^*_r=C^*$ \cite{RenaultGroupoid}.
\end{enumerate}
\end{lemma}
...
\begin{remark}[The model cannot be genuinely non-abelian]\label{VII:rem:abelian}
Lemma~\ref{VII:lem:basic}(1) is the fundamental limitation of this construction, and it is worth
...
non-abelian acting semigroup; Chapter~\ref{ch:V} shows what that costs: partitioning
range projections empty the high-temperature region.
\end{remark}

\section{Disintegration and the matrix Fourier transform}

\begin{proposition}[KMS states are scaling measures]\label{VII:prop:kmsmeasure}
...
$\Omega_S$ with
\begin{equation}\label{VII:eq:scaling}
\nu\bigl(a\cdot E\bigr)=\Ns\aaa^{-\beta}\,\nu(E)\qquad(a\in J_S),
\end{equation}
and every such $\nu$ has $V$-marginal $\pi_\beta$. Disintegrating, $\nu=\pi_\beta\otimes\rho_v$
with $(\rho_v)$ a measurable family in $\Prob(G)$, and \eqref{VII:eq:scaling} is equivalent to
\begin{equation}\label{VII:eq:equivariance}
\rho_{v+e_\pp}=\bigl(\Frob_\pp^{-1}\bigr)_*\rho_v\qquad\pi_\beta\text{-a.e., for every }\pp\in S.
\end{equation}
\end{proposition}

\begin{proof}
As in [Ch.~\ref{ch:Main}, Prop.~2.8]: the canonical expectation onto $C(\Omega_S)$ is
faithful, the KMS condition on $\mu_\aaa f\mu_\aaa^*$ gives \eqref{VII:eq:scaling}, and
conversely each scaling measure reconstructs a KMS state. The $V$-marginal is forced by
[Ch.~\ref{ch:Main}, Lem.~2.10], whose argument uses only the scaling of the valuation coordinates,
the $G$-direction being measure preserving for the translation.
Splitting \eqref{VII:eq:scaling} into its $V$- and $G$-components gives \eqref{VII:eq:equivariance}.
\end{proof}

\begin{definition}[Matrix Fourier transform]\label{VII:def:F}
...
\begin{lemma}[Matrix cohomological equation]\label{VII:lem:coho}
Equation \eqref{VII:eq:equivariance} holds if and only if for every $\rho\in\Irr(G)$
\begin{equation}\label{VII:eq:coho}
F_\rho(v+e_\pp)=\rho(\Frob_\pp)^{-1}\,F_\rho(v)\qquad\pi_\beta\text{-a.e., for every }\pp\in S .
\end{equation}
\end{lemma}

\begin{proof}
If \eqref{VII:eq:equivariance} holds then
\[
...
by Peter--Weyl, so \eqref{VII:eq:coho} for all $\rho$ determines \eqref{VII:eq:equivariance}.
\end{proof}

\begin{remark}[The inverse is not cosmetic]\label{VII:rem:inverse}
With the action of Definition~\ref{VII:def:groupoid} the transform satisfies \eqref{VII:eq:coho} with
...
Proposition~\ref{VII:prop:gauge}, the two conventions do not agree and mixing them produces a
wrong gauge.
\end{remark}

\section{What the zero-one law says, and what it does not}

\begin{proposition}[Spectral zero-one law]\label{VII:prop:zeroone}
Put $P_\rho(v)=F_\rho(v)F_\rho(v)^*$, positive semidefinite with $\|P_\rho\|\le1$. Then
\eqref{VII:eq:coho} gives
\begin{equation}\label{VII:eq:conj}
...
\begin{proof}
\eqref{VII:eq:conj} is immediate from \eqref{VII:eq:coho} and unitarity. Conjugation preserves the
spectrum, so each elementary symmetric function of the eigenvalues is invariant under the
generators of $\cR_S$, hence a.e.\ constant by ergodicity [Ch.~\ref{ch:Main}, Lem.~3.1].
\end{proof}

\begin{proposition}[$P_\rho$ need not be scalar]\label{VII:prop:notscalar}
The statement $P_\rho(v)=c_\rho I_{d_\rho}$ a.e.\ is strictly stronger than
Proposition~\ref{VII:prop:zeroone} and is false. Take $G=S_3$, $\rho$ the two-dimensional
irreducible, $\Frob_\pp=e$ for every $\pp\in S$, and
$\rho_v\equiv\lambda:=\tfrac12(\delta_e+\delta_{(12)})$, which satisfies
\eqref{VII:eq:equivariance} trivially. Then
\[
...
\begin{remark}[Numerical check]\label{VII:rem:numscalar}
...
\eqref{VII:eq:conj} requires.
\end{remark}

\begin{proposition}[What the transform sees]\label{VII:prop:gauge}
...
$\rho|_A$. Let $\Xi_\beta(A)\le\widehat A$ be as in Definition~\ref{VII:def:E} below.
\begin{enumerate}
\item Every measurable solution $F:V\to M_{d_\rho}(\C)$ of \eqref{VII:eq:coho} has the form
...
\item \eqref{VII:eq:coho} has a unitary solution $W_\rho:V\to U(d_\rho)$, unique up to right
...
\item \eqref{VII:eq:coho} has a nonzero solution if and only if some $\chi_r$ lies in
...
\begin{proof}
Put $H=WF$; then \eqref{VII:eq:coho} reads $H_{rs}(v+e_\pp)=\chi_r(\Frob_\pp)^{-1}H_{rs}(v)$ for
each entry. If $\chi_r\notin\Xi_\beta(A)$, every solution of this scalar equation vanishes
a.e.: by [Ch.~\ref{ch:Main}, Lem.~3.1] $|H_{rs}|$ is a.e.\ constant, and if the constant were
positive, [Ch.~\ref{ch:Main}, Prop.~3.3], whose proof uses only $|\chi_r(\Frob_\pp)|=1$, would make
$\sum_\pp\Ns\pp^{-\beta}|1-\chi_r(\Frob_\pp)|^2$ converge. If $\chi_r\in\Xi_\beta(A)$,
[Ch.~\ref{ch:Main}, Prop.~3.4] supplies a unimodular solution $g_r$, and $H_{rs}/g_r$ is
...
\begin{remark}[The refinement that sees $E_\beta$]\label{VII:rem:fixed}
...
and the a.e.\ constant rank of $P_\rho=F_\rho F_\rho^*$ from Proposition~\ref{VII:prop:zeroone},
for the extremal equivariant families of Theorem~\ref{VII:thm:main} below, is
...
Example~\ref{VII:ex:S3} it is $\mathbf 1\oplus\rho_2$ rather than $\mathbf 1$. It does not in
...
\begin{definition}\label{VII:def:E}
Let $A=\overline{\Frob(I_S)}$, abelian by Lemma~\ref{VII:lem:basic}(1), and let
\[
...
\begin{theorem}[Classification]\label{VII:thm:main}
The map $\varphi\mapsto\nu$ of Proposition~\ref{VII:prop:kmsmeasure} induces an affine
isomorphism
\begin{equation}\label{VII:eq:main}
...
\begin{proof}
By Proposition~\ref{VII:prop:kmsmeasure} we must describe the equivariant families $(\rho_v)$.
...
that of [Ch.~\ref{ch:Main}, Thm.~3.6] with $A$ in place of $G_S$ and $\Frob_\pp^{-1}$ in place of
...
by [Ch.~\ref{ch:Main}, Lem.~3.1, Props.~3.3, 3.4] they vanish unless $\chi\in\Xi_\beta(A)$, and the
...
is $\Prob(E_\beta\backslash G)$, giving \eqref{VII:eq:main}.
...
\begin{corollary}[Abelian case]\label{VII:cor:abelian}
If $G$ is abelian and $\Frob(I_S)$ is dense, then $A=G$, $E_\beta=\Xi_\beta^\perp$ and
\eqref{VII:eq:main} is $\Prob(G/\Xi_\beta^\perp)$, recovering [Ch.~\ref{ch:Main}, Thm.~3.6].
\end{corollary}

\section{The Tannakian obstruction does not compute the simplex}

\begin{theorem}[Discrepancy]\label{VII:thm:discrepancy}
...
be the Tannakian subgroup of Chapter~\ref{ch:VI}, for which $\cT_\beta(S)=\Rep(G/N_\beta)$. Then
\[
...
\begin{proof}
By Proposition~\ref{VII:prop:gauge}(2), $\Xi_\beta(\rho,S)<\infty$ iff $\rho|_{E_\beta}=1$, for
...
\begin{example}[Three states where the Tannakian prediction is one]\label{VII:ex:S3}
...
$\Xi_\beta(A)=\{1\}$.
\begin{itemize}
\item By Theorem~\ref{VII:thm:main} the simplex is $\Prob(E_\beta\backslash S_3)$, a
$2$-simplex: \textbf{three} extremal $\mathrm{KMS}_\beta$ states, permuted transitively by
$S_3$ with stabilizer of order $2$.
\item By Theorem~\ref{VII:thm:discrepancy}, $\cT_\beta$ consists of the representations trivial
...
\begin{remark}[Why the two disagree]\label{VII:rem:why}
...
gave no warning. The correct reading of the Tannakian theorem of Chapter~\ref{ch:VI} is therefore
...
\begin{corollary}[What is and is not an invariant]\label{VII:cor:invariant}
...
\begin{proposition}[Inner twists are invisible]\label{VII:prop:corner}
...
[Ch.~\ref{ch:Main}, \S2], and let $\alpha_\rho$ be the twisted action on $B\otimes M_n(\C)$,
\[
...
\begin{remark}\label{VII:rem:routeB}
...
Proposition~\ref{VII:prop:corner} shows that this adds nothing, for a reason more basic than
...
\Xi_\beta(\rho,S)=\infty\Rightarrow F_\rho=0&\text{false}&\text{Prop.~\ref{VII:prop:gauge}(3)}\\
F_\rho\ne0\iff\rho^{E_\beta}\ne0&\text{true}&\text{Prop.~\ref{VII:prop:gauge}, Rem.~\ref{VII:rem:fixed}}\\
\text{unitary gauge exists}\iff\Xi_\beta<\infty&\text{true}&\text{Prop.~\ref{VII:prop:gauge}(2)}\\
...
Chapter~\ref{ch:VI} is answered. The answer is not the expected one, in two ways. The simplex
is a space of probability measures on a homogeneous space rather than on a group, and the
Dirichlet obstruction data --- which is all that the $L$-functions of Chapter~\ref{ch:VI} can
...
$L$-functions must therefore contend with Corollary~\ref{VII:cor:invariant}: the $L$-functions
know $N_\beta$, and $N_\beta$ is not enough.

Three questions. First, Remark~\ref{VII:rem:fixed} identifies a refinement of the
...
as $\Xi_\beta(\rho,S)$ has in Chapter~\ref{ch:VI}, and what further datum pins down $E_\beta$ up to
conjugacy, we do not know.
Second, by
Remark~\ref{VII:rem:abelian} no construction with an abelian acting semigroup can produce a
non-abelian $E_\beta$; the interesting case $E_\beta$ non-normal already arises with
$E_\beta$ abelian and $G$ non-abelian, as in Example~\ref{VII:ex:S3}, but a theory in which the
essential-value group itself is non-abelian would need the acting semigroup to be
non-abelian, and Chapter~\ref{ch:V} shows that this empties the high-temperature region. Whether some intermediate structure --- an Ore semigroup that is non-abelian but has
...
$\pp\mapsto\Frob_\pp$; Example~\ref{VII:ex:S3} uses a constant assignment, and Chebotarev should
allow much more, along the lines of [Ch.~\ref{ch:I}, Thm.~5.1].
