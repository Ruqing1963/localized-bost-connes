% generated from papers/XII-mismatch/paper_XII_mismatch.tex -- edit the paper, then rerun assemble.py
\chapter[Multiplicity--norm mismatch and the reduction of (C4)]{The universal multiplicity--norm mismatch,\\ and the reduction of \textup{(C4)} to the norm-one subgroup}\label{ch:XII}
\renewcommand{\Nrd}{\operatorname{Nrd}}
\chapterorigin{This chapter is Paper XII of the series (\texttt{papers/XII-mismatch/}).}
\begin{summary}
Chapter~\ref{ch:XI} left two questions. We settle the first completely and reduce the
...
condition (C1) of Chapter~\ref{ch:X} requires; the Hurwitz order is one \cite{Reiner,Vigneras}. For a
...
\begin{lemma}[Local multiplicities]\label{XII:lem:split}
...
\begin{equation}\label{XII:eq:dp}
...
\begin{theorem}[Universal multiplicity--norm mismatch]\label{XII:thm:mismatch}
...
\begin{proof}
By Lemma~\ref{XII:lem:split}, for $e<d$ we have $r\ge2$ and $d_\pp-q=1+q^e+\cdots+q^{e(r-1)}-q$;
...
\begin{proposition}[Eichler and hereditary orders]\label{XII:prop:eichler}
...
primes not dividing $\mathfrak N$ the order is maximal and Theorem~\ref{XII:thm:mismatch} applies.
...
\begin{theorem}[The boundary quotient is zero]\label{XII:thm:noboundary}
...
sum of projections equal to $1$ is an orthogonal sum, as in Chapter~\ref{ch:XI}: $e_j=e_j\sum_ke_k$ gives
$\sum_{k\ne j}e_je_ke_j=0$ with positive summands, so $e_je_k=0$.
\end{proof}

\begin{remark}[Partition in mean]\label{XII:rem:betavals}
...
\begin{remark}[Why the affine systems are different]\label{XII:rem:affine}
In the $ax+b$ systems of Chapter~\ref{ch:II} the multiplicity at $\pp$ is the number
...
that $\cO_K/\pp$ has $\Ns\pp$ elements. Theorem~\ref{XII:thm:mismatch} says that the
...
\begin{corollary}[The dichotomy of Chapter~\ref{ch:XI} cannot be broken]\label{XII:cor:dichotomy}
The question of Chapter~\ref{ch:XI} --- whether some order has $d_\pp=\Ns\pp$, so that a boundary
quotient with nonempty $\mathrm{KMS}$ simplex could exist --- is answered in the negative by
Theorem~\ref{XII:thm:mismatch} and Proposition~\ref{XII:prop:eichler}, and Theorem~\ref{XII:thm:noboundary}
...
second by $e^{\beta\log\Nrd(a)}$. By Takesaki duality, as in Chapter~\ref{ch:XI}, (C4) --- non-amenability
...
\begin{proposition}[Orthogonality of the two directions]\label{XII:prop:orthogonal}
\begin{enumerate}
...
\begin{remark}[What this changes]\label{XII:rem:changes}
Proposition~\ref{XII:prop:orthogonal} is the reason the Hurwitz system is not merely a
...
\begin{proposition}[Fibre decomposition]\label{XII:prop:fibres}
...
\begin{equation}\label{XII:eq:fibre}
...
\begin{theorem}[The $\tau$-decomposition is invisible to $G^1$]\label{XII:thm:invisible}
...
\begin{equation}\label{XII:eq:reduction}
...
\eqref{XII:eq:fibre} and Proposition~\ref{XII:prop:fibres}. For \eqref{XII:eq:reduction}: if the
...
\begin{remark}[A spectral gap would not suffice]\label{XII:rem:nogap}
The converse of Kuhn's theorem fails, so the implication \eqref{XII:eq:reduction} cannot be
...
spectral gap. A gap for $\pi_0|_{G^1}$ is necessary for \eqref{XII:eq:reduction}, since
$\lambda_{G^1}$ has one, but it is the failure of temperedness that is needed.
\end{remark}

\begin{remark}\label{XII:rem:onerep}
Theorem~\ref{XII:thm:invisible} removes the direct integral from the problem. Whatever is to be
...
\begin{remark}[The failure of the expected argument]\label{XII:rem:zimmer}
...
infinite-measure-preserving essentially free action, and by Remark~\ref{XII:rem:nogap} not by a
spectral gap either.
\end{remark}

\begin{remark}[What would suffice]\label{XII:rem:suffice}
...
Koopman representation need not have. By Theorem~\ref{XII:thm:invisible} this is one
representation; by Proposition~\ref{XII:prop:orthogonal} it is measure preserving; and $G^1$
...
which is not a congruence quotient, we do not know --- Chapter~\ref{ch:XI} warns that the tree methods
behind those statements are unavailable here, and Remark~\ref{XII:rem:nogap} that a gap alone
would in any case not be enough.
\end{enumerate}
\end{remark}

\begin{remark}[Essential freeness]\label{XII:rem:free}
Route (i) needs the $G^1$-action to be essentially free, which we have not verified. The
isotropy of the $G$-action on $\widetilde\Omega$ was computed in the abelian case in Chapter~\ref{ch:II}
...
is never zero. The vanishing of the boundary quotient found in Chapter~\ref{ch:XI} for the Hurwitz
...
decomposition of $G$ given by Proposition~\ref{XII:prop:orthogonal} --- non-amenable kernel acting
measure-preservingly, abelian quotient carrying the dynamics --- is the structural feature
that distinguishes this system from every earlier one, and Theorem~\ref{XII:thm:invisible} reduces
...
that Chapter~\ref{ch:IX} found inapplicable becomes actionable; if the action is amenable, the factor
