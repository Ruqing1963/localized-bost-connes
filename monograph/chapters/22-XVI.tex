% generated from papers/XVI-transport/paper_XVI_transport.tex -- edit the paper, then rerun assemble.py
\chapter[Twists, detailed balance, and tensorisation]{Quantum optimal transport on Bost--Connes systems:\\ the twist is harmful, the generator has a sign error,\\ and the transition is invisible to local inequalities}\label{ch:XVI}
\chapterorigin{This chapter is Paper XVI of the series (\texttt{papers/XVI-transport/}).}
\begin{summary}
...
equal-norm analysis of Chapter~\ref{ch:XIV} (its Theorem 1.3). Whether it is a spectral
...
transition of Chapter~\ref{ch:IV} (its Theorem 6.2) continuous rather than first-order.
\end{summary}


\section{The twist}

We use the notation of [Ch.~\ref{ch:Main}]: $\cA_{\Q,S}=C(Y_S)\rtimes J_S$, spanned by the monomials
$\mu_\aaa f\mu_\bb^*$, with $\sigma_t(\mu_\aaa)=\Ns\aaa^{it}\mu_\aaa$; $X$ and $\nusym$ are
the compact model and its symmetric measure.Represent $\cA_{\Q,S}$ on $\cH_0=L^2(X,\nusym)\otimes\ell^2(J_S)$ in the Toeplitz
...
\begin{proposition}[The twisted commutator is unbounded]\label{XVI:prop:unbounded}
For every prime $\pp$ and every $\beta>0$,
\begin{equation}\label{XVI:eq:twist}
...
\begin{proposition}[The untwisted commutator is bounded]\label{XVI:prop:bounded}
...
on $C(Y_S)$ is the observation recorded in Chapter~\ref{ch:IV} (its Proposition 2.1),
$\Dv$ being diagonal in a basis with respect to which $C(Y_S)$ acts diagonally.
\end{proof}

\begin{remark}[Why the twist backfires]\label{XVI:rem:whybackfire}
A Connes--Moscovici twist \cite{ConnesMoscovici} repairs a \emph{multiplicative} discrepancy: it is used when
...
introduces a factor where there was none. Equation \eqref{XVI:eq:twist} is precisely the
manufactured mismatch. The twisted triple, with twist $\sigma_{i\beta}$, was the candidate
proposed in the closing assessment of Chapter~\ref{ch:IV}; this section is the reason we do not pursue
it.
\end{remark}

\begin{remark}[Numerical illustration]\label{XVI:rem:numtwist}
For $\Ns\pp=3$, $\beta=1$ the coefficient in \eqref{XVI:eq:twist} takes the values
...
horizontal operator of Chapter~\ref{ch:IV} (its Definition 2.4), amplified to act on the
...
\begin{proposition}[Kernel]\label{XVI:prop:kernel}
...
Proposition~\ref{XVI:prop:bounded}, $\ker[\Dv,\cdot\,]=\cA^\sigma_{\Q,S}$. Hence
\[
\ker L=\bigl\{x\in\cA^\sigma:[\Db,x]=0\bigr\},
\]
which equals $\C1$ if and only if $\cA^\sigma=C(Y_S)$, i.e.\ if and only if $S$ is
norm-separated in the sense of Theorem 1.3 of Chapter~\ref{ch:XIV}.
\end{proposition}
...
$\bb=(1)$, hence $f\in\C1$ by the Lip-kernel lemma of Chapter~\ref{ch:IV} (its Lemma 3.1); if
...
\begin{remark}[Spectral triple: not established]\label{XVI:rem:notriple}
...
a property of the weight sequence of Chapter~\ref{ch:IV}, which that paper does not establish and we do
...
\begin{remark}[Open Problem 1 of Chapter~\ref{ch:IV}]\label{XVI:rem:openone}
Proposition~\ref{XVI:prop:kernel} settles the kernel question but not the compact quantum metric
...
\begin{proposition}[Invariance forces the detailed-balance ratio]\label{XVI:prop:balance}
Let
\[
...
as Proposition 5.1 of Chapter~\ref{ch:III} --- whence
\begin{equation}\label{XVI:eq:inv}
...
\begin{equation}\label{XVI:eq:db}
...
$\sigma_{i\beta}(\mu_\pp)=\Ns\pp^{-\beta}\mu_\pp$. For \eqref{XVI:eq:inv} apply them termwise.
If \eqref{XVI:eq:db} holds, both brackets vanish. Conversely write
...
\begin{corollary}[The proposed generator is not invariant]\label{XVI:cor:signerror}
The choice $a_\pp=w_\pp$, $b_\pp=w_\pp\Ns\pp^{-\beta}$ gives
$b_\pp=a_\pp\Ns\pp^{-\beta}$, violating \eqref{XVI:eq:db}: the sign of the exponent is
...
$\qq$-terms of \eqref{XVI:eq:inv} cancel at $f=e_\pp$ and the value is carried by the
$\pp$-term alone:
\[
...
\begin{remark}[What the corrected ratio means]\label{XVI:rem:dbmeaning}
Condition \eqref{XVI:eq:db} reads $b_\pp/a_\pp=e^{\beta\log\Ns\pp}$: the backward rate exceeds
...
Note the price: \eqref{XVI:eq:db} requires $\sum_\pp a_\pp\Ns\pp^{\beta}<\infty$ for the second
...
\begin{theorem}[Tensorisation to an infimum]\label{XVI:thm:tensorise}
...
with the corrected rates $(a_\pp,\,a_\pp t_\pp^{-1})$ of \eqref{XVI:eq:db}. Then:
\begin{enumerate}
\item the spectral gap satisfies $\operatorname{gap}(\cL)=\inf_{\pp}a_\pp\operatorname{gap}(\cL_\pp/a_\pp)$, and by the
Poincar\'e inequality for the geometric law (Chapter~\ref{ch:IV}, its Lemma 6.1, with sharp constant
$C(t)=4t/(1-t)^2$)
\[
...
Remark~\ref{XVI:rem:dbmeaning} the infima vanish for every $\beta$, for the weight reason
...
coordinate give $\le$. The single-site gap bound is Lemma 6.1 of Chapter~\ref{ch:IV}, whose Dirichlet
form $\mathbb E_{\mu_t}|\Delta h|^2$ is that of the normalized chain with rates $(1,t^{-1})$.
\end{proof}

\begin{corollary}[The proposed asymptotic is of the wrong shape]\label{XVI:cor:wrongshape}
...
$\sum_\ell\ell^{-\beta}$ diverges there. By Theorem~\ref{XVI:thm:tensorise} the gap and MLSI
...
\begin{corollary}[Bakry--\'Emery]\label{XVI:cor:BE}
...
\begin{remark}[Why the transition is invisible]\label{XVI:rem:invisible}
...
(Chapter~\ref{ch:IV}, its Lemma 5.1). It is therefore a \emph{tail} event, detectable only by quantities
...
The same mechanism produced Theorem 6.2 of Chapter~\ref{ch:IV}, where the metric transition was
continuous rather than first-order because the order parameter is a tail observable outside
the Lipschitz domain, and Theorem 2.3 of Chapter~\ref{ch:III}, where $\Xi_\beta$ was identified as an
...
whose individual contributions are all harmless; and the Connes distance of Chapter~\ref{ch:IV} is
...
Proposition~\ref{XVI:prop:balance} satisfy KMS-symmetry in the sense of Cipriani
...
$\sigma$-algebra, where by [Ch.~\ref{ch:I}, Prop.~7.2] the order parameter lives, rather than
