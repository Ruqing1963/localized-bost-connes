% generated from papers/XXII-iwasawa/paper_XXII_iwasawa.tex -- edit the paper, then rerun assemble.py
\chapter[No $p$-adic KMS states]{Iwasawa theory and localized Bost--Connes systems:\\ there are no $p$-adic KMS states,\\ and where Leopoldt actually enters}\label{ch:XXII}
\chapterorigin{This chapter is Paper XXII of the series (\texttt{papers/XXII-iwasawa/}).}
\begin{summary}
...
through Proposition 2.2 and Remark 2.3 of Chapter~\ref{ch:II}, where the isotropy of the
...
Chapters~\ref{ch:II}, \ref{ch:III}, \ref{ch:IV}, \ref{ch:VIII}, \ref{ch:X} and \ref{ch:XXI}, referred to by numeral, are unpublished
companions; every fact used from them is restated or verified in place. [Ch.~\ref{ch:Main}] and
Chapter~\ref{ch:I} [Ch.\begin{proposition}[Positivity is unavailable]\label{XXII:prop:nostates}
...
\begin{corollary}[The KMS condition has no $p$-adic form]\label{XXII:cor:noKMS}
...
available over $\Cp$: (i) by Proposition~\ref{XXII:prop:nostates}, and (ii) because $\Cp$ has no
...
\begin{remark}[What can be transported]\label{XXII:rem:transport}
...
\begin{proposition}[$\langle\Ns\pp\rangle$ does not exist]\label{XXII:prop:undefined}
...
\begin{remark}[The localization is at the wrong set]\label{XXII:rem:wrongset}
...
That, however, runs into \S\ref{XXII:sec:noabscissa}.
\end{remark}

\section{There is no $p$-adic abscissa of convergence}\label{XXII:sec:noabscissa}

\begin{theorem}[$p$-adic Dirichlet series never converge]\label{XXII:thm:noabscissa}
...
\begin{corollary}[No $p$-adic phase transition]\label{XXII:cor:nophase}
...
[Ch.~\ref{ch:Main}], [Ch.~\ref{ch:I}] and Chapter~\ref{ch:IV} --- the competition between the decay of
...
\begin{remark}[Why $p$-adic $L$-functions look the way they do]\label{XXII:rem:whymeasures}
Corollary~\ref{XXII:cor:nophase} is not a peculiarity of Bost--Connes systems. It is the reason
...
\begin{proposition}[The element cannot generate the characteristic ideal]\label{XXII:prop:bounded}
Suppose, counterfactually to Proposition~\ref{XXII:prop:undefined}, that
...
\begin{remark}[And $\mu$]\label{XXII:rem:mu}
The proposed formula $\mu_p(K)=v_p(\mathrm{Res}_{z=0}\zeta_{p,\mathrm{KMS}}(z))$ has no
content, there being no $\zeta_{p,\mathrm{KMS}}$ by Corollary~\ref{XXII:cor:noKMS}. We note also
...
$p$-adic regulator $R_p(K)$ --- the Leopoldt quantity of \S\ref{XXII:sec:true} --- not $\mu$.
\end{remark}

\section{What is true}\label{XXII:sec:true}

\begin{proposition}[The partition function is the interpolation factor]\label{XXII:prop:euler}
...
formula for $L_p(s,\chi)$ recalled in Remark~\ref{XXII:rem:wrongset}. This is a precise
...
\begin{remark}[Finite $S$ has no thermodynamics]\label{XXII:rem:finiteS}
Consistently with this, $S=\{\pp\mid p\}$ is finite, so $\beta_c=0$ and by
the finite-generation exclusion of Chapter~\ref{ch:X} (its Remark 1.5) --- a finite generating set gives a finite obstruction sum ---
...
Corollary~\ref{XXII:cor:nophase} says the search cannot succeed in that direction.
\end{remark}

\begin{proposition}[Where Leopoldt genuinely enters]\label{XXII:prop:leopoldt}
...
in the archimedean theory: by Proposition 2.2 and Remark 2.3 of Chapter~\ref{ch:II} the isotropy of $\mathcal G_S$ off
...
\begin{remark}\label{XXII:rem:leopoldtplace}
So the intuition behind the proposal is sound --- Leopoldt does belong to this story --- but
its place is the isotropy computation of Chapter~\ref{ch:II}, not the dimension of a $p$-adic
...
\text{Leopoldt controls the system} & \textbf{true, but via Chapter~\ref{ch:II}} & \text{Prop.~5.3}\\
...
of Chapter~\ref{ch:II} (its Proposition 2.2 and Remark 2.3), which is where the units enter --- consistently with
Chapter~\ref{ch:VIII}, whose inertia model, built on the meridian direction matched to the global
units, was shown to carry no thermodynamics (its Theorem 6.1 and the moral of its
Remark 6.3), and with Chapter~\ref{ch:X} (its Remark 1.5), where the finiteness of the unit group
...
\emph{measure class} invariant of Corollary 2.4 of Chapter~\ref{ch:III} --- since that, unlike a state,
...
Remark~\ref{XXII:rem:finiteS}; is there an intermediate localization, $S$ infinite but with the
$p$-adic weights attached as a coefficient system rather than as the dynamics, in which both
structures coexist? That is the closing suggestion of Chapter~\ref{ch:XXI} (its \S5), itself pointing
at \S6 of Chapter~\ref{ch:VI}, and for the same
