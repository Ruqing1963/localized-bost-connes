% generated from papers/I-type-spectrum/paper_I_type_spectrum.tex -- edit the paper, then rerun assemble.py
\chapter[Factor type and the spectrum of transitions]{Factor type and the spectrum of transitions}\label{ch:I}
\renewcommand{\hZ}{\widehat{\mathbb{Z}}}
\chapterorigin{This chapter is Paper I of the series (\texttt{papers/I-type-spectrum/}).}
\begin{summary}
The obstruction group $\Xi_\beta(K,S)$ of [Ch.~\ref{ch:Main}] classifies the $\mathrm{KMS}_\beta$
...
We use the notation of [Ch.~\ref{ch:Main}]: $\cA_{K,S}=C(Y_S)\rtimes J_S$ with
$\sigma_t(\mu_\aaa)=\Ns\aaa^{it}\mu_\aaa$, symmetry group $G_S$, parameter homomorphism
$\sigma:I_S\to G_S$, and
\begin{equation}\label{I:eq:Xi}
...
product measure [Ch.~\ref{ch:Main}, Lem.~2.10].
...
what the factors at a given $\beta$ are. Definition~\ref{I:def:Xiext} shows the two questions
...
\begin{definition}\label{I:def:Xiext}
The \emph{extended obstruction group} is
\begin{equation}\label{I:eq:Xiext}
...
That $\Xiext$ is a subgroup follows as in [Ch.~\ref{ch:Main}, Lem.~3.5] from
$|1-z_1z_2|\le|1-z_1|+|1-z_2|$ for $|z_i|=1$; and $\Xi_\beta=\Xiext\cap(\widehat{G_S}\times\{0\})$.

\section{The type criterion}\label{I:sec:type}

Let $\beta>0$ with $\zeta_{K,S}(\beta)=\infty$, let $\varphi$ be an extremal
$\mathrm{KMS}_\beta$ state and $M=\pi_\varphi(\cA_{K,S})''$. By [Ch.~\ref{ch:Main}, Prop.~2.8 and
Lem.~2.10], $\varphi$ is given by a probability measure $\nu$ on $Y=V\times G_S$ whose
...
[Ch.~\ref{ch:Main}, Prop.~6.1], and by Krieger's theorem \cite{Krieger} the Connes invariant is
\begin{equation}\label{I:eq:SM}
S(M)=\{0\}\cup\exp(E),
\end{equation}
...
\begin{lemma}[Coboundary dictionary]\label{I:lem:dict}
...
\begin{proof}
Propositions 3.3 and 3.4 of [Ch.~\ref{ch:Main}] prove exactly this equivalence for
...
[Ch.~\ref{ch:Main}, Lem.~3.2] for $m_\pp=(1-t_\pp)/(1-c_\pp t_\pp)$, which holds for every unimodular
$c_\pp$. (Conjugating $g$ replaces $c_\pp$ by $\overline{c_\pp}$, and
$|1-c_\pp|=|1-\overline{c_\pp}|$.)
\end{proof}

\begin{proposition}[The extended group bounds the type]\label{I:prop:bound}
...
\begin{proof}
Let $(\chi,s)\in\Xiext$, let $g$ be as in Lemma~\ref{I:lem:dict}, and put
...
\]
so by \eqref{I:eq:rho} the $\T$-valued cocycle $e^{-is\rho}$ on $\cR$ is the coboundary
...
$\frac{2\pi}{c}\Z$ are $\{0\}$ and $\frac{2\pi k}{c}\Z$, $k\ge1$; by \eqref{I:eq:SM} these give
$S(M)=\{0,1\}$ and $S(M)=\{0\}\cup(e^{-2\pi k/c})^{\Z}$ respectively.
\end{proof}

\begin{remark}[$\Sigma_\beta$ and the flow of weights]\label{I:rem:flow}
...
Connes--Takesaki \cite{ConnesTakesaki} is Connes' invariant $T(M)$. We shall not need the converse
...
Proposition~\ref{I:prop:bound} is only an upper bound. Lower bounds are obtained by exhibiting
essential values directly, which is the purpose of the next lemma.
\end{remark}

\begin{lemma}[Asymptotic pairs]\label{I:lem:pairs}
Suppose $\Xi_\beta=\{1\}$, so that $\nu=\pi_\beta\otimes\mathrm{Haar}$ [Ch.~\ref{ch:Main}, Thm.~3.6].
...
each piece $D_j=\{j(v)=j\}$, and by \eqref{I:eq:rho}, $\rho(y,Ty)=\beta\log(\Ns\aaa_j/\Ns\bb_j)$
...
\section{The classical systems}\label{I:sec:classical}

\begin{proposition}[The slices for $S=P_K$]\label{I:prop:slices}
...
\begin{equation}\label{I:eq:twosums}
...
[Ch.~\ref{ch:Main}, Lem.~3.8] $\chi(\sigma_\pp)=\overline{\chi(\Frob_\pp)}$ off the conductor. For
...
\begin{theorem}[The classical systems are of type $\mathrm{III}_1$]\label{I:thm:classical}
...
\begin{proof}
$\Xi_\beta=\{1\}$ by [Ch.~\ref{ch:Main}, Cor.~3.10], so Lemma~\ref{I:lem:pairs} applies, and we must
...
\cite[Ch.~VI]{Neukirch}; for primes
...
$\sum_j\Ns\aaa_j^{-\beta}$, which diverges along the blocks used. Lemma~\ref{I:lem:pairs} gives $u\in E$
for every $u$, so $E=\R$ and $M$ is of type $\mathrm{III}_1$ by \eqref{I:eq:SM}.
\end{proof}

\begin{remark}[Two slices of one analytic input]\label{I:rem:unify}
At $\beta=1$ and $S=P_K$ the $s=0$ slice of $\Xiext$ is the uniqueness half of the classical
theorem, equivalent by [Ch.~\ref{ch:Main}, Rem.~3.11] to $L(1,\chi)\ne0$; the slices $s\ne0$ are
governed, by Proposition~\ref{I:prop:slices}, by $L(1-i\tau,\chi)\ne0$, and their triviality
is the necessary condition for type $\mathrm{III}_1$ of Proposition~\ref{I:prop:bound}(1).
...
$s\ne0$ slices are trivial is, by Remark~\ref{I:rem:flow}, the statement that the flow of
weights has no nonzero eigenvalue; that $M$ is $\mathrm{III}_1$, i.e.\ that the flow is
trivial, needed the additional input of Lemma~\ref{I:lem:pairs}.
\end{remark}

\section{Two incommensurable norms do not suffice}\label{I:sec:counter}

\begin{lemma}[No exact commensurability]\label{I:lem:noexact}
...
\eqref{I:eq:rho} in a lattice, never occurs for infinite $S$, and any failure of type
$\mathrm{III}_1$ must come from \emph{approximate} commensurability. It does.

\begin{theorem}[Exact determination of a $\mathrm{III}_{e^{-1}}$ system]\label{I:thm:counter}
Let $K=\Q$, $\beta=1$, $\|\cdot\|$ the distance to the nearest integer, and
\begin{equation}\label{I:eq:band}
...
\eqref{I:eq:band} localizes $\log\ell$ near an \emph{integer}, the rounded logarithm is
constant on each band and the exponential sums reduce to geometric series.

\begin{lemma}[Bands]\label{I:lem:bands}
...
and summing against $\ell^{-1}$ over $B_n$ gives the second. For \eqref{I:eq:err},
...
\begin{equation}\label{I:eq:absorb}
...
$W(x)=4\sqrt{\log\log x}+O(1)$ by Lemma~\ref{I:lem:bands}(4).
\end{proof}

\begin{remark}[The one delicate point]\label{I:rem:delicate}
Estimate \eqref{I:eq:err} is where care is needed: the error from linearizing $\ell^{is}$
...
Proposition~\ref{I:prop:trivialchi} and this absorption hold simultaneously: an exponent
$-\tfrac13$ would break the former and $-1$ the latter.
\end{remark}

\begin{lemma}[Cancellation in bands]\label{I:lem:SW}
...
\begin{proposition}[Non-trivial characters]\label{I:prop:chinontriv}
...
Lemma~\ref{I:lem:bands}(1) and, by Lemma~\ref{I:lem:SW} applied to the principal character, meets
...
non-principal. Now combine \eqref{I:eq:linear}, Lemma~\ref{I:lem:SW} and \eqref{I:eq:err}:
$\Lambda(x)=O(\sum_n|c_n|)+o(W(x))=o(W(x))$, so the left side of \eqref{I:eq:linear} is
$2W(x)(1+o(1))\to\infty$.
\end{proof}

\begin{proposition}[The trivial character]\label{I:prop:trivialchi}
...
(Lemma~\ref{I:lem:bands}(3)) and
$\bigl|\sum_{n\le N}e^{isn}\bigr|\le|\sin(s/2)|^{-1}<\infty$, Dirichlet's test gives
convergence of $\sum_ne^{isn}w_n$. With \eqref{I:eq:err}, $\Lambda(x)=O_s(1)+o(W(x))=o(W(x))$
and the sum diverges as before.
\end{proof}

\begin{lemma}[Matched pairs in consecutive bands]\label{I:lem:matched}
...
$\ell'\notin\{a,b\}$ [Ch.~\ref{ch:Main}, Lem.~2.10(1)], is $\equiv1\pmod{\ell'^{\,k}}$; so every
...
\]
and $\sum_nw_n=\infty$ by Lemma~\ref{I:lem:bands}(4).
\end{proof}

\begin{proof}[Proof of Theorem~\ref{I:thm:counter}]
$\zeta_S(1)=\infty$ is Lemma~\ref{I:lem:bands}(4). $S$ is infinite, so
Lemma~\ref{I:lem:noexact} supplies two primes with irrationally related logarithms.
Propositions \ref{I:prop:chinontriv} and \ref{I:prop:trivialchi} give
$\Xi^{\mathrm{ext}}_1=\{1\}\times2\pi\Z$, hence $\Xi_1=\{1\}$ and $\Sigma_1=2\pi\Z$.
Proposition~\ref{I:prop:bound}(3) with $c=2\pi$ gives $E\subseteq\Z$: $M$ is of type
$\mathrm{III}_{e^{-k}}$ for some $k\ge1$ or of type $\mathrm{III}_0$, and in particular not
of type $\mathrm{III}_1$. Since $\Xi_1=\{1\}$, Lemma~\ref{I:lem:pairs} applies, and
Lemma~\ref{I:lem:matched} supplies pairs with $\beta\log(a_j/b_j)\to1$ ($\beta=1$),
$\sigma_{b_j}\sigma_{a_j}^{-1}\to1$ and $\sum a_j^{-1}=\infty$; hence $1\in E$, so $E=\Z$,
$S(M)=\{0\}\cup e^{\Z}$ by \eqref{I:eq:SM}, and $M$ is of type $\mathrm{III}_{e^{-1}}$.
\end{proof}

\begin{remark}[What the proof does not use]\label{I:rem:tools}
...
in \eqref{I:eq:linear} is over the \emph{integers} $n$, not over primes: its partial sums are
...
\begin{remark}[Numerical check]\label{I:rem:numtype}
The asymptotic regime of \eqref{I:eq:band} begins at $\varepsilon_n<\tfrac12$, i.e.\
$\ell>e^{e^4}\approx10^{23}$, and is beyond direct computation. The mechanics of
Lemmas~\ref{I:lem:bands}(3) and \ref{I:lem:SW} were instead verified with a fixed band
...
\begin{corollary}[Independence of the two classifications]\label{I:cor:indep}
Uniqueness of the $\mathrm{KMS}_\beta$ state and type $\mathrm{III}_1$ are logically
independent: Theorem~\ref{I:thm:counter} has the first and fails the second, while
Theorem~\ref{I:thm:classical} together with [Ch.~\ref{ch:Main}, Cor.~3.10] has both. Equivalently,
$\Xiext$ can be trivial on the axis $s=0$ and nontrivial off it.
\end{corollary}

\section{The spectrum of transitions}\label{I:sec:spectrum}

We now turn to the horizontal direction: which chains $\{\Xi_\beta\}_{\beta>0}$ occur. By
[Ch.~\ref{ch:Main}, Lem.~3.5] the chain is non-decreasing, and by [Ch.~\ref{ch:Main}, Prop.~4.16] any
finite chain is realizable. We realize arbitrary countable chains.

\begin{theorem}[Realization]\label{I:thm:realize}
...
\begin{lemma}[Independence of the quadratic conditions]\label{I:lem:disjoint}
...
\begin{definition}[Block scheme]\label{I:def:blocks}
...
for all $i<j$ (such primes have positive density by Lemma~\ref{I:lem:disjoint}, so they exist in
...
\]
each of density $2^{-j}$ by Lemma~\ref{I:lem:disjoint}, and
\[
...
\begin{lemma}[The pieces]\label{I:lem:pieces}
...
Lemma~\ref{I:lem:disjoint} gives positive density, so enlarging $z_{j+1}$ further (finitely
...
\begin{proof}[Proof of Theorem~\ref{I:thm:realize}]
(1) is Lemma~\ref{I:lem:pieces}(1). For (2), fix a modulus $M$ supported on a finite
$F\subset S$, let $Q_F=Q\cap\{q:q\mid M\}$ and
$K_M=\{a:\bigl(\tfrac{a}{q}\bigr)=1\ \forall q\in Q_F\}$, a subgroup of index $2^{|Q_F|}$
with $(\Z/M\Z)^\times/K_M\cong(\Z/2\Z)^{Q_F}$. By Lemma~\ref{I:lem:pieces}(1) every $a\in K_M$
...
$a\in K_M$ with $\tilde\chi(a)\ne1$; by Lemma~\ref{I:lem:pieces}(1)
...
\]
by Lemma~\ref{I:lem:pieces}(2). This is the stated formula; for $\beta>1$,
$\zeta_S(\beta)<\infty$ and [Ch.~\ref{ch:Main}, Lem.~3.5] gives $\Xi_\beta=\widehat{G_S}$.
\end{proof}

\begin{remark}[Numerical check]\label{I:rem:numcascade}
...
Lemma~\ref{I:lem:disjoint} occur with densities in $[0.0621,0.0629]$ against the predicted
...
$\beta$-weighted sums collapsing across $\beta=b$ as Lemma~\ref{I:lem:pieces}(2) requires.
\end{remark}

\section{Cantor transition loci, and the unattainable staircase}\label{I:sec:cantor}

\begin{definition}\label{I:def:locus}
...
\begin{corollary}[Dense and Cantor loci]\label{I:cor:loci}
Taking $(\beta_n)$ an enumeration of $\Q\cap(0,1)$ in Theorem~\ref{I:thm:realize} gives
...
\begin{proof}
By Theorem~\ref{I:thm:realize}(3), $\Xi_\beta$ is constant on an open interval
...
Remark~\ref{I:rem:whatremains}. Finally $\beta=1$ always lies in $\mathcal L(S)$, since
$\Xi_\beta$ jumps to $\widehat{G_S}$ there.
\end{proof}

\begin{definition}\label{I:def:order}
For weights $w_n>0$ with $\sum w_n=1$ the \emph{normalized rank} is
$\dw_w(\beta)=\sum_{n:\beta_n<\beta}w_n\in[0,1]$.
\end{definition}

\begin{theorem}[Pure-jump staircase]\label{I:thm:staircase}
In the situation of Corollary~\ref{I:cor:loci} with locus $\cC$, $\dw_w$ is non-decreasing,
...
\begin{theorem}[The devil's staircase is unattainable]\label{I:thm:impossible}
...
countable locally compact group $\mathbb I_S$ [Ch.~\ref{ch:Main}, Prop.~2.5]; the dual of a compact
...
Theorem~\ref{I:thm:realize}, $\chi_n\notin\Xi_{\beta_n}$ by the endpoint divergence in
Lemma~\ref{I:lem:pieces}(2) while $\chi_n\in\Xi_\beta$ for $\beta>\beta_n$, so the jump at
...
\begin{theorem}[Approximation]\label{I:thm:approx}
Let $\cC$ be the middle-thirds Cantor set, $c$ the Cantor function, and $S$ as in
Corollary~\ref{I:cor:loci} with $(\beta_n)$ dense in $\cC$. For every $\varepsilon>0$ there
...
\begin{remark}[Jump set versus transition locus]\label{I:rem:whatremains}
The obstruction in Theorem~\ref{I:thm:impossible} is not an artifact of the construction of
\S\ref{I:sec:spectrum}: $\Xi_\beta$ is a subgroup of the fixed countable group
$\widehat{G_S}$, and a monotone family of subgroups of a countable group can change at only
countably many parameters. What survives, and what Corollary~\ref{I:cor:loci} realizes, is the
...
$\ell_0\ne q_n$ [Ch.~\ref{ch:Main}, proof of Prop.~4.12], so the first factor is multiplied by
...
is $+1$ by the choice of $Q$ in Definition~\ref{I:def:blocks}, or the block property gives
...
$\beta>\beta_n$ by Lemma~\ref{I:lem:pieces}(2); apply the two Borel--Cantelli lemmas to the
...
The bijection is Theorem~\ref{I:thm:realize}(3) together with [Ch.~\ref{ch:Main}, Thm.~3.6], which
identifies the extreme points with $\widehat{\Xi_\beta}=(\Z/2\Z)^{\{n:\beta_n<\beta\}}$.
\end{proof}

\begin{remark}[Numerical check]\label{I:rem:numpsi}
...
\begin{remark}[Physical reading]\label{I:rem:memory}
Each $\Psi_n$ is a tail observable: changing the valuations at any finite set of primes
leaves it unchanged, because a change at $\ell\in R_n$ flips two factors of \eqref{I:eq:Psin}
...
Theorem~\ref{I:thm:impossible} they cannot condense continuously: each bit is a discrete
...
\text{factor type} & \text{bounded above by}\ \Sigma_\beta,\ \text{below by pairs} & \text{Prop.~\ref{I:prop:bound}, Lem.~\ref{I:lem:pairs}}\\
S=P_K,\ \beta\le1 & \mathrm{III}_1;\ \Sigma_\beta=\{0\}\ \text{via}\ L(1-i\tau,\chi)\ne0 & \text{Thm.~\ref{I:thm:classical}, Prop.~\ref{I:prop:slices}}\\
\text{two incommensurable norms} & \text{do not suffice} & \text{Thm.~\ref{I:thm:counter}}\\
\text{exact commensurability} & \text{impossible for infinite }S & \text{Lem.~\ref{I:lem:noexact}}\\
\text{uniqueness}\Rightarrow\mathrm{III}_1 & \text{false} & \text{Cor.~\ref{I:cor:indep}}\\
\text{chains}\ \{\Xi_\beta\} & \text{any}\ (\beta_n)\subset(0,1)\ \text{realizable} & \text{Thm.~\ref{I:thm:realize}}\\
\text{transition locus} & \text{any Cantor set} & \text{Cor.~\ref{I:cor:loci}}\\
\text{jump set} & \text{always countable} & \text{Thm.~\ref{I:thm:impossible}}\\
\text{devil's staircase} & \text{unattainable, approximable} & \text{Thms.~\ref{I:thm:impossible}, \ref{I:thm:approx}}
\end{array}
\]

Five questions remain. First, whether $\Sigma_\beta$ determines the type, i.e.\ whether the
flow of weights of these factors always has pure point spectrum (Remark~\ref{I:rem:flow}); if
it does, Proposition~\ref{I:prop:bound} is an equivalence and Lemma~\ref{I:lem:pairs} is not
needed. Second, the extended group $\Xiext$ has not been computed for the sparse sets
of [Ch.~\ref{ch:Main}, \S5]; in particular whether Brun-summable $S$ gives $\mathrm{III}_\lambda$ or
$\mathrm{III}_0$ is open. In Theorem~\ref{I:thm:counter} we determined $\Sigma_1$ exactly, but
for a general band set with $\varepsilon(\ell)=(\log\log\ell)^{-\theta}$ the dependence of
$\Sigma_1$ on $\theta$ is unexamined. Fourth, Theorem~\ref{I:thm:impossible} shows the
...
know. Finally, Lemma~\ref{I:lem:dict} says that $\Xi_\beta$ is the kernel of
