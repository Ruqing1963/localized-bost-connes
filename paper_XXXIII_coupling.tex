...
\title[The adaptive coupling upper bound]{The adaptive coupling upper bound for the Wasserstein separation:\\ microscopic flips against the macroscopic symmetry cost}
\author{Ruqing Chen}
\address{GUT Geoservice Inc., Montreal, Canada}
\email{ruqing@hotmail.com}
\thanks{Paper XXXIII of the series on localized Bost--Connes systems. The framework is
that of \cite{Main}, cited by DOI; Paper XXIX of the series is referred to by numeral,
and the setting is restated in place.}
\date{5 September 2026}

\begin{document}

\begin{abstract}
In the setting of Paper XXIX --- extreme broken-phase KMS states $\varphi_\pm$ separated
...
recursion. On the toy model of Paper XXIX's numerical experiment the bound evaluates to
...
\section{Setting, cost, and the two exact identities}\label{sec:setting}

We keep the notation of Paper XXIX: $X=G\times\prod_{j\in J}\overline\N$,
...
\begin{lemma}[The shift identity]\label{lem:shift}
...
shift of Lemma~\ref{lem:shift} (direction $+1$ on $s=+1$ fibres, $-1$ on $s=-1$ fibres;
...
\section{The adaptive coupling and the bound}\label{sec:bound}

\begin{theorem}[Adaptive coupling upper bound]\label{thm:upper}
...
\]
the upper bound pinches against the squeeze lower bound $w_0\Pi_\infty$ of Paper XXIX in
the deep-freeze limit.
\end{theorem}

\begin{proof}
Apply Lemma~\ref{lem:residual} at $j_1$: a coupling moving mass $1-m_{j_1}=\tau_{j_1}$ by
one $j_1$-step (cost $\le w_{j_1}$ each, Lemma~\ref{lem:shift}) and leaving a residual
...
\begin{proposition}[Optimal enumeration]\label{prop:order}
The bound of Theorem~\ref{thm:upper} is minimized by enumerating $J$ in increasing order
of $w_j$.
\end{proposition}
...
\begin{remark}[Against the numerical verdict, and the remaining gap]\label{rem:numerics}
On the toy model of Paper XXIX's experiment ($w_0=1$, $w_j=(j+1)^{-1}$,
