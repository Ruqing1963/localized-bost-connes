...
\title[The gap is exactly one]{Localized $GL_2$ Bost--Connes systems:\\ the equilibrium range and the automorphic obstruction range\\ are disjoint, by exactly one}
\author{Ruqing Chen}
\address{GUT Geoservice Inc., Montreal, Canada}
\email{ruqing@hotmail.com}
\thanks{Sources, code and data for the entire series: \url{https://github.com/Ruqing1963/localized-bost-connes}; this paper is in \texttt{papers/XXI-GL2/}.}
\date{}
...
\]
matching Theorem 4.1 of Paper VI of the series at $n=2$; the term
...
$p+1$ while the norm is $p$, and by Theorem 1.2 of Paper XII this excess is universal:
...
$SL_2(\Z)$, so by the unit-group exclusion of Paper X (its Remark 1.5) its partition function
...
The framework is that of \cite{Main}; Papers VI, X and XII of the series, referred to by
numeral, are unpublished companions, and every fact used from them is restated or verified
in place.

\begin{proposition}[The semigroup crossed product is unavailable]\label{prop:units}
...
replaced by the Hecke-algebra quotient of Connes--Marcolli \cite{CM}, in which one sums over
$GL_2(\Z)$-orbits.
\end{proposition}

\begin{proof}
This is the unit-group exclusion recorded in Paper X (its Remark 1.5): a multiplicative norm
is $1$ on units, so an infinite unit group makes
$\zeta_P$ diverge identically.
\end{proof}

\begin{proposition}[The Solomon zeta]\label{prop:solomon}
...
\begin{equation}\label{eq:zetaGL2}
...
equivalently per sublattice $L\subseteq\Z_p^2$, weighted by its index \cite{CM} --- not one
...
\begin{lemma}[Hilbert--Schmidt identity]\label{lem:HS}
...
\]
so that
\begin{equation}\label{eq:XiGL2}
\Xi_\beta(\pi_f,S)=8\sum_{p\in S}p^{-\beta}\sin^2\!\bigl(\theta_p/2\bigr).
\end{equation}
\end{lemma}

\begin{proof}
Lemma 2.2 of Paper VI (its Trace form) with $n=2$, together with $\Tr U_p=a_p/\sqrt p=2\cos\theta_p$.
\end{proof}

\begin{theorem}[The $L$-function dictionary, corrected]\label{thm:L}
For $\beta>1/2$,
\begin{equation}\label{eq:Lcorrect}
\Xi_\beta(\pi_f,S)=4\log\zeta_S(\beta)-2\,\Re\log L_S(\beta,\pi_f)+O(1),
\end{equation}
...
\begin{proof}
By \eqref{eq:XiGL2}, $\Xi_\beta=4\sum_pp^{-\beta}-2\sum_pp^{-\beta}\Tr U_p$. The first sum is
$\log\zeta_S(\beta)+O(1)$; the second is $\Re\log L_S(\beta,\pi_f)+O(1)$, the prime-power
terms converging for $\beta>1/2$. This is Theorem 4.1 of Paper VI at $n=2$, where the general
form is $2n\log\zeta_{K,S}(\beta)-2\Re\log L_S(\beta,\rho)+O(1)$.
\end{proof}

\begin{remark}[Why $\Sym^2$ cannot appear]\label{rem:sym2}
...
\begin{corollary}[Divergence at $\beta=1$]\label{cor:diverge}
...
pole --- so $\Re\log L_S(\beta,\pi_f)=O(1)$. Apply \eqref{eq:Lcorrect}.
\end{proof}
...
\begin{theorem}[Disjoint ranges]\label{thm:gap}
...
\begin{proof}
(1) By \eqref{eq:XiGL2}, $\Xi_\beta(\pi,S)\le8\sum_{p\in S}p^{-\beta}$, finite for
$\beta>\beta_c(S)$. (2) By Proposition~\ref{prop:solomon}, convergence is that of
...
\begin{corollary}[The programme cannot close]\label{cor:cannot}
...
system has no Gibbs states, and Corollary~\ref{cor:diverge}, though true, has no
...
\begin{remark}[The shift is the multiplicity--norm mismatch]\label{rem:mismatch}
The displacement by $1$ is the phenomenon isolated in Theorem 1.2 of Paper XII. There, for a
...
factor $(1-p^{1-\beta})^{-1}$ in \eqref{eq:zetaGL2} and shifts the abscissa by $1$. In
Paper XII the excess emptied the boundary quotient; here it displaces the equilibrium
range out of reach of the obstruction. It is one fact with two consequences.
\end{remark}

\section{Sparse sets for $\Delta$}

\begin{theorem}[Anomalous sets exist]\label{thm:sparse}
...
$2\cdot4^{-j}\sum_{p\in S_j}p^{-1}\le4\cdot4^{-j}$. This is Theorem 5.3 of Paper VI with
$\Delta$ in place of an elliptic curve.
\end{proof}

\begin{remark}[But not symmetry breaking]\label{rem:notbreaking}
Theorem~\ref{thm:sparse} produces a set on which the analytic obstruction is finite. It does
not produce a broken phase, for two reasons already recorded. By
Remark 5.4 of Paper VI an obstruction group is not a broken phase until an algebra realizes
it; and by Theorem~\ref{thm:gap} the temperature $\beta=1$ at which the construction operates
...
Paper VI: the Hilbert--Schmidt series, its $L$-function form once the spurious
...
Paper XII showed to be universal.
...
Proposition 1.4 of Paper X norms factor through the abelianization, and on $GL_2$ orbits the
...
acting semigroup --- which is the route of \S6 of Paper VI, where the Kakutani analysis
applies verbatim and only the existence of a tractable expectation was left open. We would
pursue the second.

\begin{thebibliography}{9}
\bibitem{CM} A. Connes, M. Marcolli, \emph{Noncommutative Geometry, Quantum Fields and Motives}, AMS Colloq. Publ. 55, 2008.
