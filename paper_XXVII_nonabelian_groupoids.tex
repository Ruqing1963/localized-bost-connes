...
\hypersetup{pdftitle={Non-abelian Bost-Connes groupoids: two no-go theorems, and a construction that realizes Prob(G/N) exactly when the Frobenius lifts mix},pdfauthor={Ruqing Chen}}

\title[Non-abelian Bost--Connes groupoids]{Non-abelian Bost--Connes groupoids:\\ two no-go theorems, and a construction that realizes $\Prob(G/N_\beta)$\\ exactly when the Frobenius lifts mix}
\author{Ruqing Chen}
\email{ruqing@hotmail.com}
\thanks{Sources, code and data for the entire series: \url{https://github.com/Ruqing1963/localized-bost-connes}; this paper is in \texttt{papers/XXVII-nonabelian-groupoids/}.}
\date{}
...
subgroup $N_\beta=\bigcap\{\ker\rho:\Xi_\beta(\rho,S)<\infty\}$ of Paper VI is the canonical
non-abelian replacement of the obstruction group, and Paper VII showed that the obvious
...
proposed in Paper VI for closing the gap and settle both.
...
state by \cite[Lem.~2.10]{Main}; $\cR_S$ is the tail equivalence relation on $V$ (finitely
many coordinates changed), ergodic by the zero--one law \cite[Lem.~3.1]{Main};
...
theorem of Paper VI of the series, and $N_\beta$ depends only on the classes.

\emph{The series.} The foundational paper is \cite{Main}; Paper I \cite{SeriesI} treats the
factor type. Papers VI (the Tannakian obstruction) and VII (the Galois-twisted groupoid)
are in preparation; we recall from them only what is needed and prove everything used.
...
points \cite[Thm.~3.6]{Main}. The target of a non-abelian theory is a system whose
$\mathrm{KMS}_\beta$ simplex is $\Prob(G/N_\beta)$ with $G$ acting transitively on the
extreme points by translation, stabilizer $N_\beta$. The model of Paper VII --- $I_S$
...
Two ways around this were proposed in Paper VI (its Remark 6.1) and left open in Paper VII:
...
\section{Route B: noncommutative coefficient algebras}\label{sec:B}
...
\begin{theorem}[Route B is abelian]\label{thm:B}
\begin{enumerate}
...
$T_\beta=\Prob(G_S/\Xi_\beta^\perp)$ \cite[Thm.~3.6]{Main}: the abelian answer. (c) In every
...
\cite[Prop.~2.8]{Main}, and for general $\cB$ there may be additional $\mathrm{KMS}_\beta$
states, all of them tracial on the fixed-point algebra. None of this reintroduces the
representation: the matrix Kakutani dichotomy of Paper VI, valid for commuting lifts,
...
\section{Route A, first reading: isotropy cannot carry $N_\beta$}\label{sec:iso}
...
groupoid). Neshveyev's theorem \cite[Thm.~1.3]{Neshveyev} describes the $\mathrm{KMS}_\beta$
...
\begin{theorem}[No-go for isotropy]\label{thm:isoNoGo}
...
$\varphi_\mu$ is a $\mathrm{KMS}_\beta$ state by \eqref{eq:nesh}, and it is extreme because
...
\cite[Prop.~2.8]{Main}, and all of its multiplicity comes from the ergodic decomposition
of the unit space. That is where a non-abelian construction must put it.
\end{remark}

\section{Route A, second reading: the ordered Frobenius cocycle}\label{sec:ordered}
...
this is $u_j$ and $c$ is the constant-increment cocycle of Paper VII, but in general the
increments at different coordinates do not commute, and \eqref{eq:c} is the way to have a
cocycle nonetheless: the obstruction of Paper VI, that a cocycle with \emph{constant}
...
\]
continuous on $X$ and extending \eqref{eq:Pn}--\eqref{eq:incr}. Let $\widetilde X=\varinjlim(X,+a)$
...
translation $R_h(x,g)=(x,gh)$, commuting with \eqref{eq:action}, hence on $\cA_c$ by
...
the KMS states, the argument of \cite[Prop.~2.8]{Main} applies verbatim (or one may invoke
\eqref{eq:nesh}): a $\mathrm{KMS}_\beta$ state is $\nu\circ E$ for a scaling measure $\nu$
...
$\pi_\beta$ by \cite[Lem.~2.10]{Main}. Disintegrating and comparing the two sides of the
...
\begin{theorem}[The simplex of the ordered model]\label{thm:simplex}
...
\begin{proof}
By Lemma~\ref{lem:basic} we classify the equivariant families \eqref{eq:equiv}, i.e.\ the
...
For commuting lifts $f$ can be taken trivial and $H=E_\beta$, recovering Paper VII. The
...
\begin{theorem}[Non-abelian Kakutani]\label{thm:kakutani}
...
so $K_n$ is $\cF_{>n}$-measurable \cite[Lem.~3.1]{Main} and independent of $P_n$, which is
...
and by \eqref{eq:minv}, iterating $m_{i}^{-*}(\,\cdot\,)m_i^{-1}\le\lambda_i(\,\cdot\,)$ with
...
limit $\ge1$ by \eqref{eq:det} and $\Xi_\beta<\infty$, so $\det F\ne0$ a.s.\ and $F$ is
...
$m\to\infty$ by \eqref{eq:det}, while $\|C_m\|_{\HS}\le\E\|K_m\|_{\HS}=\sqrt d$; so
...
\begin{corollary}[$N_\beta$ is the normal closure of the Mackey range]\label{cor:normalclosure}
...
$\cR_S\ltimes_{c'}H$. So by Theorem~\ref{thm:kakutani}, $\Xi_\beta(\rho,S)<\infty$ iff
...
The rest is Theorem~\ref{thm:simplex}.
\end{proof}

\subsection{When is the Mackey range normal? A mixing criterion}

\begin{theorem}[Mixing forces $H=G$]\label{thm:mixing}
...
Theorem~\ref{thm:kakutani}, $\E[B|\cF_{\le n}]=P_nC_n$ with $C_n=\E[P_n^{-1}B]$ satisfying
\eqref{eq:consist} and $\|C_m\|_{\HS}\le\sqrt d\,\|B\|_\infty$. By hypothesis
...
skew product means $H=G$, and Theorem~\ref{thm:simplex} gives the rest.
\end{proof}
...
\begin{theorem}[The simplex depends on the lift]\label{thm:lift}
...
$H=A$. Theorem~\ref{thm:simplex} gives $\Prob(A\backslash S_3)$, three points.

(2) We verify the hypothesis of Theorem~\ref{thm:mixing}. For the sign character,
...
$\ell_{2k}$ making an angle of $60^\circ$; by \eqref{eq:mean}, $m_i=Q_i+\lambda_iQ_i^\perp$
...
$\sum_kt_{2k}\ge\frac12\sum_{i\ge2}t_i=\infty$. Theorem~\ref{thm:mixing} applies.
...
\begin{corollary}[No canonical non-abelian model]\label{cor:nocanonical}
...
i.e.\ the Tannakian data of Paper VI.
\end{corollary}
...
and use the lifts $k_iu_ik_i^{-1}$ in \eqref{eq:c}. The extra coordinates are not moved by
...
\begin{proposition}[Rigid Tannakian subgroups]\label{prop:rigid}
...
\begin{proof}
By Corollary~\ref{cor:normalclosure}, $H\le N_\beta$ and $\langle\langle H\rangle\rangle=N_\beta$;
the hypothesis leaves only $H=N_\beta$.
\end{proof}

\begin{example}[A nontrivial $\Prob(G/N_\beta)$ realized for every lift]\label{ex:A3}
...
$N_\beta=A_3$, of prime order; by Proposition~\ref{prop:rigid}, for every lift the ordered
...
used as in the constructions of \cite[\S5]{SeriesI}, supplies such splittings from an
...
\text{B: }\cB\rtimes J_S,\ \cB\text{ noncommutative} & \text{no-go: KMS states are traces on }\cB & \ref{thm:B}\\
\text{A: isotropy}=N_\beta & \text{no-go: gives }\Irr(N_\beta)\text{ and a fixed point} & \ref{thm:isoNoGo}\\
\text{A: skew product by }c & \Prob(H\backslash G),\ H\text{ the Mackey range} & \ref{thm:simplex}\\
\rho\circ c\ \text{coboundary}\iff\Xi_\beta(\rho,S)<\infty & \text{true, every lift} & \ref{thm:kakutani}\\
N_\beta=\langle\langle H\rangle\rangle;\ \Prob(G/N_\beta)\iff H\trianglelefteq G & \text{true} & \ref{cor:normalclosure}\\
\text{mixing means}\Rightarrow H=G & \text{true} & \ref{thm:mixing}\\
\text{simplex independent of the lift} & \text{false }(S_3) & \ref{thm:lift}\\
\Prob(G/N_\beta)\ \text{for every lift} & N_\beta\text{ rigid, e.g. }A_3 & \ref{prop:rigid},\ \ref{ex:A3}
\end{array}
\]
...
produces the symmetry breaking: Theorems \ref{thm:B} and \ref{thm:isoNoGo} are no-go
...
when the lifts are chosen to commute as in Paper VII, exactly when the abelian
...
\bibitem{Neshveyev} S. Neshveyev, \emph{KMS states on the $C^*$-algebras of non-principal groupoids}, J. Operator Theory 70 (2013), 513--530.
