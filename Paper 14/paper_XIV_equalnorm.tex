...
\title[The equal-norm case]{The equal-norm case: norm-separation is not a hypothesis\\ but a characterization, and the gauge-equivariant repair}
\author{Ruqing Chen}
\address{GUT Geoservice Inc., Montreal, Canada}
\email{ruqing@hotmail.com}
\thanks{Sources, code and data for the entire series: \url{https://github.com/Ruqing1963/localized-bost-connes}; this paper is in \texttt{papers/XIV-equalnorm/}.}
\date{}

\begin{document}

\begin{abstract}
Paper XIII of the series proved $\cA_{K,S}^\sigma=C(Y_S)$ (its Theorem 1.4) under the
...
reconstruction of Paper XIII (its Theorem 2.5) holds for every number field and every prime
...
recovered in Paper XIII either.

Finally we record a limitation of Paper XIII that we had not noticed there: every $K\ne\Q$
has infinitely many split primes, so $P_K$ is \emph{never} norm-separated. The theorem of
Paper XIII therefore covers the classical full prime set only for $K=\Q$.
\end{abstract}
...
We use the notation of the foundational paper \cite{Main}: $\cA_{K,S}=C(Y_S)\rtimes J_S$ is
...
$c(\aaa\bb^{-1},y)=\log\Ns\aaa-\log\Ns\bb$ implementing the flow. Papers II, III and XIII of
...
which Paper III of the series sees that $\{\sigma_t\}$ has compact closure).

\begin{lemma}[The diagonal is the gauge fixed-point algebra]\label{lem:gauge}
...
Lemma~\ref{lem:gauge}. Conversely if $L\ne0$, pick $0\ne c\in L$ and write $c=\aaa-\bb$ with
$\aaa,\bb\in J_S$ coprime; then $\Ns\aaa=\Ns\bb$ and $\mu_\aaa\mu_\bb^*\in\cA^\sigma$ is not
in $C(Y_S)$, the two ideals being distinct.
\end{proof}

\begin{corollary}[Nothing to remove]\label{cor:nothing}
The hypothesis of Theorem 1.4 of Paper XIII is not a sufficient condition that might be
...
equal norm $p$. So $P_K$ is not norm-separated, and Theorem 1.4 of Paper XIII applies to
the full prime set only when $K=\Q$.
\end{proposition}

\begin{remark}[Numerical illustration]\label{rem:num}
The first split prime is $5$ for $\Q(i)$, $7$ for $\Q(\sqrt{-3})$ and $\Q(\sqrt2)$, and $11$
for $\Q(\sqrt5)$; in each case two primes of norm $p$. This is a limitation of Paper XIII
...
\begin{proposition}[The proposed stratification is not a direct sum]\label{prop:notsum}
...
\begin{theorem}[The correct structure]\label{thm:structure}
...
\begin{proof}
$H=\overline{\iota(\R)}$ has annihilator $L$ by the proof of Theorem~\ref{thm:equiv}, so
...
\begin{remark}[Property A]\label{rem:A}
...
Lemma~\ref{lem:gauge} this is correct as a \emph{description} --- $C(Y_S)$ is the range of the
...
determine when $L\ne0$. By Theorem~\ref{thm:equiv}(3), $\sigma$ determines only
$H=L^\perp\subsetneq\T^S$. Invoking $\gamma$ to recover $C(Y_S)$ therefore assumes what is
missing.
\end{remark}

\begin{remark}[Property B]\label{rem:B}
...
Cartan subalgebra is Theorem 2.5 of Paper II of the series (topological principality; the
argument is reproduced inside the proof of Theorem~\ref{thm:uncond} below). That it is the
...
which is what Theorem 1.4 of Paper XIII was designed to supply and what is at issue here.
...
\begin{remark}[What we do not know]\label{rem:open}
...
\begin{definition}\label{def:gaugeequiv}
...
coordinates. Theorem~\ref{thm:uncond} produces a bijection, and shows that $\theta$ was
induced by one after all.
\end{definition}

\begin{theorem}[Unconditional reconstruction]\label{thm:uncond}
...
\end{enumerate}
These are the conclusions of Theorem 2.5 of Paper XIII, with no norm-separation required
anywhere; what is conditional in (5) is conditional there too.
\end{theorem}

\begin{proof}
\emph{The Cartan pair.} By Lemma~\ref{lem:gauge}, $C(Y_{S_i})=\cA_i^{\T^{S_i}}$, and
...
this reproduces Theorem 2.5(1) of Paper II of the series. Renault's theorem \cite{Renault}
...
Lemma 2.1 of Paper XIII run with $\widetilde c$ in place of the scaling cocycle: there the
...
to $\mathrm{KMS}_\beta$ states; by \cite[Thm.~3.6]{Main} the simplices are
...
which the simplex changes. Finally (5) is part (6) of Theorem 2.5 of Paper XIII: its proof
...
gauge-equivariance and Lemma~\ref{lem:gauge} --- and its Lemma 2.1 --- replaced by the
$\widetilde c$ argument above; its Remark 2.6 records that these are the only two entry
points.
\end{proof}

\begin{corollary}[Dedekind zeta]\label{cor:zeta}
...
among the conclusions: $h^+_S$ is recovered by no argument of Paper XIII (its Remark 3.2),
and nothing here adds one.
\end{corollary}

\begin{remark}[Is the hypothesis reasonable?]\label{rem:reasonable}
...
gauge-equivariant. What Theorem~\ref{thm:uncond} does not do is deduce the conclusion from
$\sigma$-equivariance alone, and by Corollary~\ref{cor:nothing} no argument through
$\cA^\sigma$ ever will.
\end{remark}
...
removed from Theorem 1.4 of Paper XIII, because Theorem~\ref{thm:equiv} shows it is that
...
$S=P_K$ that Proposition~\ref{prop:PK} shows was previously excluded --- with what was
...
Theorem~\ref{thm:structure} is a new object in this programme, being the first non-commutative
