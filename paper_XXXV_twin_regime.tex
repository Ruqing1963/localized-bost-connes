...
\title[The free twin-prime system is supercritical]{The free twin-prime Bost--Connes system is unconditionally supercritical:\\ resolving the phase dichotomy via exact partial sums}
\author{Ruqing Chen}
\address{GUT Geoservice Inc., Montreal, Canada}
\email{ruqing@hotmail.com}
\thanks{Paper XXXV of the series on localized Bost--Connes systems. The framework is that
of \cite{Main}, cited by DOI; Papers V and XXIV of the series are referred to by numeral,
and the facts used are restated in place. Script and output in
\texttt{code/twin-regime/} of the repository.}
...
phase dichotomy of Paper V is decided by the fugacity at the classical critical point:
...
\section{The dichotomy and the identity}\label{sec:identity}

Let $S\subseteq\mathcal{P}$ and let $\mathcal T\mathcal C(S)$ be the free variant of Paper V: the
...
\]
Thus a free equilibrium exists iff $L_S(\beta)<1$, and Paper V's trichotomy at the
classical critical point $\beta_c(S_{\mathrm{twin}})=1$ (Brun-summable, closed classical
phase, Paper XXIV) reads: \emph{Regime (S)} if $L_S(1)>1$, with
...
heights. Conditionally, \eqref{eq:brun} gives $L_S(1)\approx0.9511+0.1080=1.059$; but a
conditional inequality decides nothing.

\section{Unconditional resolution via segmented sieving}\label{sec:sieve}
...
\section{Methodological remarks: why heuristics do not bypass the sieve}\label{sec:method}

\begin{remark}[Occupancy does not imply divergence]\label{rem:occupancy}
...
(Paper XXIV); but occupancy is a statement about \emph{which} classes carry primes, not
...
\begin{remark}[Metaphor is not proof]\label{rem:metaphor}
...
Theorem~\ref{thm:main}; everything sharper is conditional, and is labeled so.
\end{remark}
