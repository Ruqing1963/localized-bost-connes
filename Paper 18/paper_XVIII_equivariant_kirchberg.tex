...
\title[The compact group advantage is illusory]{Circle actions on function field Bost--Connes systems:\\ the compact group advantage is illusory,\\ and the Weil zeta function is thermodynamic}
\author{Ruqing Chen}
\address{GUT Geoservice Inc., Montreal, Canada}
\email{ruqing@hotmail.com}
\thanks{Sources, code and data for the entire series: \url{https://github.com/Ruqing1963/localized-bost-connes}; this paper is in \texttt{papers/XVIII-equivariant-kirchberg/}.}
\date{}

\begin{document}

\begin{abstract}
In \S7 of Paper XVII of this series we suggested that the periodicity of the modular flow over a function
...
$\Sp_p(\sigma)=q^{d_S\Z}$ of Proposition 1.1 of Paper XV. The class group contributes
...
thermodynamic, not $K$-theoretic --- which is the principle of Corollary 2.4 of Paper III
appearing once more.
\end{abstract}
...
The framework is that of \cite{Main}, transported to function fields as in Paper XVII of the
series, whose notation we keep. Papers II, III, IX, XIII, XV and XVII, referred to by
numeral, are unpublished companions; every fact used from them is restated or verified in
place.

\begin{proposition}[Degeneration for $S=\cP_K$]\label{prop:infinity}
...
affine system
$\cA^{\mathrm{aff}}=C(\widehat{\cO_S})\rtimes(\cO_K\rtimes J_S)$ of Paper II (its \S6)
...
\begin{corollary}[The set-up must be affine]\label{cor:affine}
...
\begin{proposition}[Takai duality lands outside the Kirchberg class]\label{prop:takai}
...
\begin{proposition}[The action is not Rokhlin]\label{prop:notrokhlin}
...
\begin{remark}[Withdrawal of a suggestion]\label{rem:withdraw}
We suggested there that periodicity of $\sigma$ ``may bring Remark 5.1 of Paper XIII within
...
Remark 5.1 of Paper XIII, that flows on Kirchberg algebras are not classified, stands and
is not circumvented by compactness.
\end{remark}

\section{What the equivariant $K$-theory records}

\begin{proposition}[The cyclic module]\label{prop:KT}
...
\begin{corollary}[No genus]\label{cor:nogenus}
The module of Proposition~\ref{prop:KT} records the single quantity $q^{d_S}$, which is
exactly the content of the point spectrum $\Sp_p(\sigma)=q^{d_S\Z}$ already available from
$(\cA,\sigma)$ by Proposition 1.1 of Paper XV. It contains no information about the genus, the
Weil numbers, or the point counts.
\end{corollary}

\begin{remark}[What $K$-theory does see]\label{rem:whatKsees}
By Theorem 3.3 of Paper II the class of the unit in $K_0$ of the boundary records the
...
\begin{theorem}[$Z_C$ as a thermodynamic quantity]\label{thm:partition}
...
$\beta>1$ is Corollary 2.2 of Paper XVII.
\end{proof}

\begin{theorem}[Complete geometric reconstruction]\label{thm:reconstruct}
...
point spectrum $\Sp_p(\sigma)=q^{d_S\Z}$ (Proposition 1.1 of Paper XV, computed for these
systems in Remark 4.3 of Paper XVII), with $d_S=1$ for $S=\cP_K$ by F.~K. Schmidt
(Proposition 1.1 of Paper XVII), and then
$P(T)=Z_C(T)(1-T)(1-qT)$. The remaining items are the Weil conjectures for curves.
\end{proof}

\begin{example}[Numerical illustration]\label{ex:elliptic}
...
\end{array}
\]
The $b_n$ are the multiplicities in the partition function \eqref{eq:partition}; the point
counts and the Jacobian order follow from them.
\end{example}

\begin{remark}[The principle, again]\label{rem:principle}
Theorem~\ref{thm:reconstruct} delivers the conclusion asked for while
Corollary~\ref{cor:nogenus} shows the proposed route cannot. This is the pattern established
in Corollary 2.4 of Paper III: the arithmetic of these systems lives in the equilibrium data
...
Remark 4.5 of Paper II, Proposition 3.1 of Paper III and Theorem 2.1 of Paper IX.
\end{remark}
...
\text{\S7 of Paper XVII suggestion} & \textbf{withdrawn} & \text{Rem.~2.3}\\
...
function but non-isomorphic Jacobians exist; by Theorem~\ref{thm:reconstruct} their systems
