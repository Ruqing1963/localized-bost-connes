...
\title[The ray-class repair for affine systems]{The ray-class repair for affine systems:\\ the class group as the index of definable scalings}
\author{Ruqing Chen}
\address{GUT Geoservice Inc., Montreal, Canada}
\email{ruqing@hotmail.com}
\thanks{Paper XXXI of the series on localized Bost--Connes systems. The framework is that
of \cite{Main}, cited by DOI; Papers II (published) and XXX of the series are referred to
by numeral, and every fact used from them is restated in place.}
\date{5 September 2026}

\begin{document}

\begin{abstract}
Paper XXX computed the K-theory of the affine boundary quotient at a principal prime and
located the class group ``at the entrance'': for non-principal $\pp$ the affine monoid of
Paper II (its \S6) is not well defined over $\OK$. This paper performs the repair and
...
Pimsner--Voiculescu computation of Paper XXX applies verbatim with $(\pi,q^h)$ in place of
...
statement of Paper II must read $(\Ns\pp^{h}-1)[1]=0$ in the repaired system; the
...
\section{The bottleneck, and the scaling-index theorem}\label{sec:index}
...
affine monoid of Paper II (its \S6) acts by $x\mapsto\gamma x+b$ with $b\in\OK$ and
...
\begin{theorem}[The class group is the index of definable scalings]\label{thm:index}
The set of depths $j$ for which the affine system of Paper II's \S6 admits a well-defined
...
$\sum_{a\in\OK/\pp^{h}}\mu_a\mu_a^*=1$; it reduces to the system of Paper XXX when $h=1$.
...
\begin{proof}
Lemma~\ref{lem:exist} gives the depths. For the last statement, a local uniformizer
...
\begin{remark}
Theorem~\ref{thm:index} upgrades the observation of Paper XXX --- ``the class group is at
...
Problem (ii) of \S\ref{sec:verdict}.
\end{remark}

\section{K-theory of the repaired boundary}\label{sec:ktheory}

\begin{theorem}\label{thm:ktheory}
...
dense in $K_\pp$; and the exterior-tower computation of Paper XXX applies verbatim with
...
exact order $\Ns\pp^{\,h}-1$. The divisibility statement of Paper II's \S6 must
...
those of Paper XXX, restated there in full; the only inputs were a global generator of the
...
\begin{example}[$\Q(\sqrt{-5})$ at the non-principal prime above $3$]\label{ex:sqrt5}
...
\begin{remark}[Separating $q$ from $h$: the middle layers generically do]\label{rem:separate}
...
$\Z/2$ of Example~\ref{ex:sqrt5}. So Problem (i) of \S\ref{sec:verdict} is partially
...
\begin{remark}[Unit-orbit sensitivity persists]\label{rem:units}
As in Paper XXX, replacing $\pi$ by $u\pi$ for a unit $u$ of norm $1$ changes the middle
...
\section{The arithmetic verdict, and what remains}\label{sec:verdict}

The class group enters the K-theory as \emph{torsion, not rank}: the free ranks of
Theorem~\ref{thm:ktheory} are those of Paper XXX, unchanged, while the order of the
distinguished class $[1]$ is promoted from $q-1$ to $q^{h}-1$. Three problems remain.
\emph{(i) Separating $q$ from $h$:} partially resolved by Remark~\ref{rem:separate} ---
