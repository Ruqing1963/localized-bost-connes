...
\title[Galois descent for affine boundaries]{Galois descent for affine Bost--Connes boundaries:\\ capitulation, the obstruction cocycle, and the emergence of the class group}
\author{Ruqing Chen}
\address{GUT Geoservice Inc., Montreal, Canada}
\email{ruqing@hotmail.com}
\thanks{Paper XXXII of the series on localized Bost--Connes systems. The framework is that
of \cite{Main}, cited by DOI; Papers XXX and XXXI of the series are referred to by
numeral, and every fact used from them is restated in place.}
...
$\ord[1]=q^{h}-1$ at the bottom ($q=\Ns\pp$; Paper XXXI) and
...
\section{The capitulated system and the Galois action}\label{sec:top}
...
\begin{lemma}[Splitting structure]\label{lem:split}
...
\begin{proposition}[The top system, and why the units are forced]\label{prop:top}
...
twist is a coboundary --- which, by Theorem~\ref{thm:obstruction}, happens exactly when
$\pp$ is principal.
\end{proposition}

\section{The obstruction cocycle is the class}\label{sec:obstruction}

\begin{theorem}[Emergence of the full class group]\label{thm:obstruction}
...
\section{Descent: norm, imprimitivity, and the ladder of unit classes}\label{sec:descent}

\begin{proposition}[The bottom scaling is the norm of the top scaling]\label{prop:norm}
$\bigl(N_{H/K}(\Pi)\bigr)=\pp^{\,|\Cl_K|}=(\pi)^{|\Cl_K|/h}$, so
$N_{H/K}(\Pi)=\pi^{\,g}u$ for a unit $u\in\OK^\times$: the Paper XXXI scaling $\pi$ is,
...
\begin{theorem}[Imprimitivity, and the equivariant K-theory reduced]\label{thm:imprimitivity}
...
(Lemma~\ref{lem:split}), Green's imprimitivity theorem \cite{Green} gives a strong Morita
equivalence
\[
...
not completed here (Problem (i) of \S\ref{sec:remaining}).
\end{theorem}

\begin{theorem}[The ladder of unit classes]\label{thm:ladder}
...
\begin{proof}
The bottom is Paper XXXI. For the top, the $r=0$ layer of the exterior-tower argument of
Paper XXX applies on $X_H$: $C(X_H)=\varinjlim_mC(\OH/(\Pi^m))$ with transitive
translation action, $|\OH/(\Pi)|=q^{|\Cl_K|}$ by Lemma~\ref{lem:split}, so
...
$K_*(\partial\Aloc_{\PP_1}\rtimes\Z/h)$: the exterior tower of Paper XXX
...
\emph{(ii)} The fixed-point question: the Paper XXXI bottom system is the $C$-fixed-point
