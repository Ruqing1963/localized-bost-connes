...
\title[A non-commutative Ore LCM semigroup: the Hurwitz quaternions]{A non-commutative Ore LCM semigroup\\ with arithmetic thermodynamics:\\ the Hurwitz quaternions}
\author{Ruqing Chen}
\address{GUT Geoservice Inc., Montreal, Canada}
\email{ruqing@hotmail.com}
\thanks{Sources, code and data for the entire series: \url{https://github.com/Ruqing1963/localized-bost-connes}; this paper is in \texttt{papers/X-semigroup-search/}.}
\date{}
...
erases the arithmetic entirely (Paper IX of the series). Additive structure makes the range
projections partition, so the Cuntz relation forces $\beta=1$ and destroys the
high-temperature phase structure (Paper II). A free non-commutative monoid has orthogonal
ranges, whence $\sum_\pp\Ns\pp^{-\beta}\le1$ and no equilibrium state exists above the critical
point at all (Paper V).
...
but not sufficient for escaping Paper IX: what matters there is amenability of the
...
Condition (C4) is settled, in the negative, in Paper XXVIII of the series: the orbit
...
Papers II, V, VII, IX and XXVII, cited by numeral, are in preparation, and nothing proved
here depends on them.

\begin{definition}[The target]\label{def:target}
...
\begin{proposition}[(C2) follows from (C1)]\label{prop:C2}
...
\begin{remark}[Re-diagnosis of the free monoid]\label{rem:free}
Proposition~\ref{prop:C2} shows that (C2) costs nothing once (C1) is imposed, and identifies
what actually goes wrong in the free variant of Paper V. A free monoid is right LCM in the
...
\begin{proposition}[Norms factor through the abelianization]\label{prop:ab}
...
\begin{remark}[What (C1)--(C3) already exclude]\label{rem:selection}
Two consequences narrow the search sharply, and we record them because they explain why the
candidate of \S\ref{sec:hurwitz} is arithmetic rather than combinatorial. If $P$ is finitely
generated then the obstruction series of \cite[(1.1)]{Main}, which runs over generators, is a
...
Proposition~\ref{prop:ab} its abelianization must carry infinitely many multiplicatively
independent norm values --- the analogue of the $\Q$-linear independence of the $\log\Ns\pp$
used throughout \cite{SeriesI}.
\end{remark}

\section{The Hurwitz quaternion monoid}\label{sec:hurwitz}
...
\begin{theorem}[$P$ satisfies \textup{(C1)--(C3)}]\label{thm:hurwitz}
\begin{enumerate}
...
\]
Identity \eqref{eq:zeta} was checked against partial sums to $n=20000$, agreeing to eight
...
\begin{remark}[The Toeplitz relations]\label{rem:relations}
...
\]
the third by Proposition~\ref{prop:C2}, together with unitarity of $v_u$ for the $24$ units.
...
Proposition~\ref{prop:C2} no Cuntz relation constrains $\beta$: the
high-temperature region $\beta<2$ is not excluded on those grounds, in contrast with both the
affine systems of Paper II and the free variant of Paper V. The ring $C^*$-algebra of
...
\begin{proposition}[The group of fractions is non-amenable]\label{prop:nonamen}
...
\begin{remark}[Multiplicity at each prime]\label{rem:mult}
...
$a_p/|P^\times|=24(1+p)/24=p+1$ by Theorem~\ref{thm:hurwitz}(6).
...
$(1-2^{1-\beta})$ in \eqref{eq:zeta}, and verified numerically alongside
$p+1$ for $p=3,5,7,11,13,17,19,23$.

Two consequences. The Kakutani analysis of \cite[\S3]{Main} was carried out with the
...
\begin{remark}[Ramanujan graphs]\label{rem:ramanujan}
...
\begin{remark}[What decided the collapse in Paper IX]\label{rem:omitted}
Conditions (C1)--(C3) are necessary but not sufficient, and they do not address the property
that governed Paper IX. There the factor was erased not because the index semigroup was
...
Proposition~\ref{prop:nonamen} supplies non-amenability of $G$, which is necessary; it is far
...
applies verbatim as in Paper IX, collapsing the factor to $R_\infty$, $R_\lambda$ or a
Krieger factor.
\end{remark}

\begin{definition}[The fourth condition]\label{def:C4}
\begin{enumerate}
...
\begin{remark}[Status, and how it might go]\label{rem:status}
...
a Toeplitz algebra is not a boundary. The nested projections of Proposition~\ref{prop:C2}
...
the multiplicity of Remark~\ref{rem:mult}: the $p+1$ right ideals of norm $p$ have no
counterpart in \cite{Main}, and the Kakutani criterion must be re-derived before the
classification can be transported. Third, if (C4) holds, whether the resulting obstruction
group is genuinely non-abelian, or whether the mechanism of Paper VII --- that the image of
a commutative index set in the symmetry group is abelian --- reappears in some form despite
the non-commutativity of $P$; Paper XXVII shows that non-abelian symmetry breaking to a
...
and \eqref{eq:zeta} is a classical Solomon-type zeta function. What is claimed is the
selection: that the requirements of \S1 admit this model, with the properties computed in
\S\ref{sec:hurwitz}. Whether the $\mathrm{KMS}$ classification of $\cT(\cH\setminus\{0\})$ is
