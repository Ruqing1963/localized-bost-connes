...
# ------------------------------------------------------------------ Paper IV
print("Paper IV  (metric geometry: single-prime model of Remark 6.4)")
try:
    import scipy.optimize as so
    def paperIV_W1(t, wr, K):
        mu = (1 - t) * t ** np.arange(K); c = 2 * mu * (-1) ** np.arange(K)
        def g(h):
            d = np.append(h[1:] - h[:-1], 0.0); return 1 - 4 * h ** 2 - d ** 2 / wr ** 2
        def jg(h):
            n = len(h); J = np.zeros((n, n)); d = np.append(h[1:] - h[:-1], 0.0); idx = np.arange(n)
            J[idx, idx] = -8 * h + 2 * d / wr ** 2; J[idx[:-1], idx[:-1] + 1] = -2 * d[:-1] / wr ** 2; return J
        best = -1.0
        for start in (np.zeros(K), 0.3 * (-1) ** np.arange(K), 0.4 * np.ones(K)):
            r = so.minimize(lambda h: -c @ h, start, jac=lambda h: -c, constraints=[{'type': 'ineq', 'fun': g, 'jac': jg}],
                            method='SLSQP', options={'maxiter': 3000, 'ftol': 1e-12})
            if g(r.x).min() > -1e-8: best = max(best, -r.fun)      # accept any feasible point (SLSQP may report failure at the optimum)
        return best
    for t, wr, claimed, K in [(0.5, 1, 0.707, 40), (0.9, 1, 0.707, 120), (0.1, 1, 0.838, 30), (0.5, 5, 0.981, 40), (0.9, 5, 0.981, 120)]:
        v = paperIV_W1(t, wr, K)
        report("IV", f"W_1 for t={t}, w_r={wr} (w_0=1)", claimed, round(v, 3), abs(v - claimed) < 0.002)
    # the alternating profile value (w_0^-2 + w_r^-2)^(-1/2) and the Kakutani lower bound (1-t)/(1+t)
    report("IV", "alternating-profile value for w_r=1 and 5", (0.707, 0.981), (round((1 + 1) ** -0.5, 3), round((1 + 1 / 25) ** -0.5, 3)), True)
except ImportError:
    print("  scipy not available; Paper IV check skipped")

# ------------------------------------------------------------------ Paper V
print("Paper V  (free critical exponents, Remarks 4.4 and 5.3)")
try:
    from scipy.special import exp1
    from scipy.optimize import brentq
    X = 20_000_000
    P, _ = primes_upto(2 * X + 1)
    isp = np.zeros(2 * X + 2, dtype=bool); isp[P] = True
    pr = P[P <= X].astype(float)
    def Lfree(beta, pset, k=1, C=1.0):
        s = np.sum(pset ** (-beta))
        if k == 1: return s + (exp1((beta - 1) * math.log(X)) if beta > 1 else np.inf)
        u = np.linspace(math.log(X), math.log(X) + 60, 200001)
        return s + C * np.trapezoid(np.exp((1 - beta) * u) / u ** 2, u)
    def root(pset, k=1, C=1.0): return brentq(lambda b: Lfree(b, pset, k, C) - 1, 1.0001, 3, xtol=1e-9)
    for name, pset, claimed in [("all primes", pr, 1.399433), ("except 2", pr[pr > 2], 1.224388), ("except 2,3", pr[pr > 3], 1.154890),
                                ("p>1e6", pr[pr > 1e6], 1.019162), ("p>1e2", pr[pr > 100], 1.056773), ("p>1e4", pr[pr > 1e4], 1.028710)]:
        v = root(pset); report("V", f"beta_free({name})", claimed, round(v, 6), abs(v - claimed) < 2e-6)
    for N, claimed in [(10, 1.147338), (100, 1.356444), (1000, 1.388390), (1e4, 1.396110), (1e6, 1.399069), (1e7, 1.399307)]:
        q = pr[pr <= N]; v = brentq(lambda b: np.sum(q ** (-b)) - 1, 1.0, 3); report("V", f"root of sum_(p<={N:g}) p^-beta = 1", claimed, round(v, 6), abs(v - claimed) < 2e-6)
    v = brentq(lambda b: exp1((b - 1) * math.log(1e8)) - 1, 1.0001, 2); report("V", "p>1e8 (tail integral only)", 1.014372, round(v, 6), abs(v - 1.014372) < 2e-6)
    C2 = 0.6601618158
    pi = P[P <= X]; sg = pi[isp[2 * pi + 1]].astype(float); tw = pi[isp[pi + 2]].astype(float)
    report("V", "Sophie Germain: L(1) bare + 2C_2/log x", 1.535, round(np.sum(1 / sg) + 2 * C2 / math.log(X), 3), abs(np.sum(1 / sg) + 2 * C2 / math.log(X) - 1.535) < 2e-3)
    report("V", "Sophie Germain: beta_free (HL tail)", 1.2114, round(root(sg, 2, 2 * C2), 4), abs(root(sg, 2, 2 * C2) - 1.2114) < 2e-4)
    report("V", "twin primes: bare L(1) to 2e7", 0.9805, round(np.sum(1 / tw), 4), abs(np.sum(1 / tw) - 0.9805) < 2e-4)
    report("V", "twin primes: L(1) with HL tail", 1.059, round(np.sum(1 / tw) + 2 * C2 / math.log(X), 3), abs(np.sum(1 / tw) + 2 * C2 / math.log(X) - 1.059) < 2e-3)
    report("V", "twin primes: beta_free (HL tail)", 1.0089, round(root(tw, 2, 2 * C2), 4), abs(root(tw, 2, 2 * C2) - 1.0089) < 2e-4)
    del P, isp, pr
except ImportError:
    print("  scipy not available; Paper V check skipped")
